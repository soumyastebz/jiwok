	...
	*/
	...
	if(trim($_REQUEST['keyword'])){ 
	 ...
	 $cleanData	=	str_replace("'",'\\\\\\\\\'',trim($_REQUEST['keyword']));
		$cleanData	=	str_replace("%"," ",trim($cleanData));//echo trim($_REQUEST['keyword']);exit;
		...
		$errMsg = "Special characters are not allowed";
			...
			// $searchQuery	= " AND (d.`program_title` LIKE '%".$cleanData."%' OR m.`program_id` LIKE  '%".$cleanData."%' )"; changed inorder to correct special character issue.
			 $searchQuery	= " AND (d.`program_title` LIKE '%".trim($_REQUEST['keyword'])."%' OR m.`program_id` LIKE  '%".trim($_REQUEST['keyword'])."%' )";
...
<? if($_POST['update1']){
 ...
 $result = $GLOBALS['db']->query($sql);}
          ...
          }header('Location: http://10.0.0.8/jiwok/admin/program_img.php');
	  }?>
...
-->
						<td valign="middle" class="extraLabels"  align="right">Keyword&nbsp;<input name="keyword" type="text" size="10" value="<?=mb_convert_encoding($objGen->_output($_REQUEST['keyword']),'UTF-8','UTF-8');?>">&nbsp;<input type="button" name="search" onClick="javascript:this.form.submit();" value="Search"></td>
						</tr></table>
					</TD>
					</TR>
					 ...
					 <?php }
						...
						?>
							...
							<?if($row['image_newdesign']==''){?>
                             <TD align="center"><input type="hidden" name="image_<?php echo $key;?>" id="image_<?php echo $key;?>" ></TD>
...
<input type="button" name="crop" id="crop" onClick=" return myFunction1('<?php echo $key;?>')" value="Crop" ><?} else{?>
	<a href="crop/crop.php?image=<?php echo $row['image_newdesign']; ?>&prg_id=<?php echo $row['program_id']; ?>" title="Previous image">Crop</a>
	<?}?>
...
{  
	...
	var prg_id = document.getElementById('pgm_id_'+res).value ;
	    window.location ='../admin/crop/crop.php?image='+image+'&prg_id='+prg_id;
