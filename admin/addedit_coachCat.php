...
*****************************************************************************/
	...
	*/
	...
	if($_POST['add']||$_POST['update']){
		...
		if($_POST['add'])	{ 
			...
			while(list($key,$value) = each($languageArray)){ 
				...
				$check	= $objCoachCat->_isCoachCatExists($objGen->_clean_data($_REQUEST['service_content'.$key]));
				if($check) 
		...
		if($_POST['update'])	{
			...
			while(list($key,$value) = each($languageArray)){
				...
				$check = false;
				$check	= $objCoachCat->_isCoachCatExists($objGen->_clean_data($_REQUEST['service_content'.$key]),$_REQUEST['coachCatId']);
				if($check) 
	...
	if($_POST['add']){
		...
		if(count($errorMsg)==0)	{
			$nextId = $objCoachCat->_insertMaster($_POST['service_status']);
			reset($languageArray);
			while(list($key,$value) = each($languageArray)){
				...
				$result =  $objCoachCat->_insertCoachCat($elmts,count($languageArray));
			}
			header("Location:list_caochCat.php?status=success_add&langId=".$_REQUEST['langId']."&pageNo=".$_REQUEST['pageNo']."&maxrows=".$_REQUEST['maxrows']."&type=".$_REQUEST['type']."&field=".$_REQUEST['field']."&keyword=".$_REQUEST['keyword']);
		}
	}
	...
	if($_POST['update']){
		...
		if(count($errorMsg)==0)	{
			reset($languageArray);
			while(list($key,$value) = each($languageArray)){
					$query = "SELECT  count(*) as dataCount FROM coach_category_manager WHERE coach_categorymaster_id =".$_REQUEST['coachCatId']." AND language_id=".$key."";
					...
					if(DB::isError($result)) 
						...
						if($coundRecs >= 1){
							...
							$result = $objCoachCat->_updateCoachCat($_REQUEST['coachCatId'],$key,$elmts);
						...
						else
						{
						    $title	= $_POST['service_content_'.$key];
							$body	= $_POST['service_description_'.$key];
							$result = $objCoachCat->_insertOneCoachCat($_REQUEST['coachCatId'],$key,$title,$body);
						}
				
			}
			//print_r($title);
			
			header("Location:list_caochCat.php?status=success_update&langId=".$_REQUEST['langId']."&pageNo=".$_REQUEST['pageNo']."&maxrows=".$_REQUEST['maxrows']."&type=".$_REQUEST['type']."&field=".$_REQUEST['field']."&keyword=".$_REQUEST['keyword']);
	...
	if($_REQUEST['coachCatId'] and count($errorMsg)==0){
		...
		$result = $objCoachCat->_getAllById($_REQUEST['coachCatId']);
...
<tr> 
              ...
              <tr> 
				...
				<form name="serviceform" action="addedit_coachCat.php" method="post" onSubmit="return formChecking()">
                            ...
                            <TD valign="top">
								  ...
								  <tr>
									...
									<TD align="left">
										...
										<table height="50" class="topActions"><tr><td valign="middle" width="50"><a href="list_caochCat.php?pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$_REQUEST['keyword']?>"><img src="images/list.gif" border="0" alt="Listing Record">&nbsp;List   </a></td>
