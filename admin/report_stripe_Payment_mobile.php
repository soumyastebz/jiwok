	...
	*/
	...
	if($tab == 2)
	{
		 $query		=	"SELECT count(*) AS total_condition	FROM stripe_transaction AS PC INNER JOIN payment AS 	
					     ...
					     c.brand_master_id=b.brand_master_id WHERE PS.payment_status='0' AND (PS.version = 'mobile_stripe') AND  
	...
	elseif($tab == 3)
	{
		 $query		=	"SELECT count(distinct(PC.user_id)) AS total_condition	FROM stripe_transaction AS PC INNER JOIN payment AS 
		    			PS ON PC.payment_id = PS.payment_id INNER JOIN user_master AS UM ON UM.user_id = PC.user_id 
		    			left join brand_user b on b.user_id=UM.user_id left join brand_master c on 
		    			c.brand_master_id=b.brand_master_id WHERE PS.payment_status='1' AND (PS.version = 'mobile_stripe') AND 
	...
	elseif($tab == 4)
	{
		if($page_name!= "")
		{
					...
					$query		=	"SELECT count(*) AS total_condition	FROM payment As PS INNER JOIN user_master AS UM ON UM.user_id = PS.payment_userid  
		                           ...
		                           ( PS.version = 'mobile_stripe')  $whereSql $joinquery ";  
		...
		else
		{
	                $query		=	"SELECT count(*) AS total_condition	FROM stripe_transaction AS ST 
							        ...
							        WHERE UM.user_status !=0 AND ST.status='PAID' and p.version='mobile_stripe' $whereSql $joinquery";
		}
	}
	elseif($tab == 5)
	{
		
		 //~ echo $query		=	"SELECT count(*) AS total_condition	FROM stripe_payment AS PP INNER JOIN user_master AS UM 		
						//~ ON UM.user_id = PP.user_id left join brand_user b on b.user_id=UM.user_id left join          		
								//~ brand_master c on c.brand_master_id=b.brand_master_id WHERE UM.user_status !=0 AND            
								    			//~ PP.status	!=	'ACTIVE'  $whereSql $joinquery";
...
JOIN payment AS P ON P.payment_userid = PP.user_id
WHERE UM.user_status !=0
AND PP.status != 'ACTIVE'
AND P.version = 'mobile_stripe' $whereSql $joinquery";
	...
	if($total_condition['total_condition']>0){
	    ...
	    $sql ="SELECT UM.user_fname, UM.user_lname,UM.user_email,PC.*,PS.payment_currency,PS.payment_amount,PS.payment_date
		      ...
		      ( PS.version = 'mobile_stripe') AND PC.status ='CANCELLED' $whereSql $joinquery";
		}
		elseif($tab == 3)
		{
		 $sql = "SELECT UM.user_fname, UM.user_lname,UM.user_email,PC.* 					
					...
					left join brand_user b on b.user_id=UM.user_id 
					left join brand_master c on c.brand_master_id=b.brand_master_id
					WHERE PS.payment_status='1' AND (PS.version = 'mobile_stripe') AND PC.status='REFUND'
					$whereSql $joinquery
					";
		}
		elseif($tab == 4)
		{
			if($page_name!= "")
			{
				  ...
				  */
					...
					As PS INNER JOIN user_master AS UM ON UM.user_id = PS.payment_userid left join brand_user b on b.user_id=UM.user_id 
					left join brand_master c on c.brand_master_id=b.brand_master_id
					WHERE UM.user_status !=0 AND PS. payment_status	=1 AND (PS.version = 'mobile_stripe')  $whereSql $joinquery ";
			}
			else
			{
				/*
				 ...
				 */
		 $sql = "SELECT UM.user_fname, UM.user_lname,UM.user_email,ST.join_date,ST.status as 'payment_status',ST.brand,p.version 					
		...
		elseif($tab == 5)
		{
			...
			$sql = "SELECT UM.user_fname, UM.user_lname,UM.user_email,PP.unsubscribed_date,PP.status,PP.brand ,	P.payment_userid,P.version				
					FROM stripe_payment AS PP															
					INNER JOIN user_master AS UM ON UM.user_id = PP.user_id 
					left join brand_user b on b.user_id=UM.user_id 
					left join brand_master c on c.brand_master_id=b.brand_master_id
					JOIN payment AS P ON P.payment_userid = PP.user_id
					WHERE UM.user_status !=0 AND PP.status	!=	'ACTIVE' AND P.version = 'mobile_stripe' $whereSql $joinquery";
		...
		else
		{
		 $sql = "SELECT UM.user_fname, UM.user_lname,UM.user_email,PS.payment_amount,PS.payment_currency,
					...
					left join brand_user b on b.user_id=UM.user_id 
					left join brand_master c on c.brand_master_id=b.brand_master_id
					WHERE PS.payment_status='1' AND (PS.version = 'mobile_stripe') AND PC.status = 'PAID' $whereSql $joinquery
					";	
		}
	...
	//$sql = $sqlStart.$fromSql.$whereSql." GROUP BY user_master.user_id ORDER BY user_master.user_doj DESC";
		...
		{   
			 $paging_query	= "SELECT count(PC.user_id) 
					...
					left join brand_user b on b.user_id=UM.user_id 
					left join brand_master c on c.brand_master_id=b.brand_master_id
					WHERE PS.payment_status='0' AND (PS.version = 'mobile_stripe'
		...
		elseif($tab == 3)
		{
			$paging_query	= "SELECT count(PC.user_id) 
					...
					left join brand_user b on b.user_id=UM.user_id 
					left join brand_master c on c.brand_master_id=b.brand_master_id
					WHERE PS.payment_status='1' AND (PS.version = 'mobile_stripe'
		...
		elseif($tab == 4)
		{
			if($page_name!= "")
				{				
					...
					$paging_query	= "SELECT count(PS.payment_userid),PS.payment_status 
					FROM payment AS PS									
					INNER JOIN user_master AS UM ON UM.user_id = PS.payment_userid
					left join brand_user b on b.user_id=UM.user_id 
					left join brand_master c on c.brand_master_id=b.brand_master_id
					WHERE UM.user_status !=0 AND PS. payment_status	=1 AND (PS.version = 'mobile_stripe'
                    )  $whereSql $joinquery";	
				}
				else
				{
					$paging_query	= "SELECT count(ST.user_id),ST.status as 'payment_status'
					FROM stripe_transaction AS ST										
					INNER JOIN user_master AS UM ON UM.user_id = ST.user_id 
					left join brand_user b on b.user_id=UM.user_id 
					left join brand_master c on c.brand_master_id=b.brand_master_id
...
<select name="toY">
  <?
	...
	<ul>		
        <li><a href="report_stripe_Payment_mobile.php?pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=1"><span>Payments</span></a></li>
		<li><a href="report_stripe_Payment_mobile.php?pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=2"><span>Cancel</span></a></li>
		<li><a href="report_stripe_Payment_mobile.php?pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=3"><span>Refund</span></a></li>
		<li><a href="report_stripe_Payment_mobile.php?pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=4"><span>Members</span></a></li>
        <?php if($page_name== "")
			  {?>
					
        <li><a href="report_stripe_Payment_mobile.php?pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=5"><span>Unsubscribed </span></a></li><?php } ?>
			  ...
			  $amountQuery	=	"SELECT sum( PS.payment_amount ) AS Total, PS.payment_currency FROM  		 		
 											...
 											INNER JOIN user_master AS UM ON UM.user_id = PC.user_id WHERE                    	
 																PS.payment_status='0' AND (PS.version = 'mobile_stripe'
					  ...
					  $PlanQuery	=	"SELECT sum( PS.payment_amount ) AS Total,COUNT( PS.payment_amount )  																				 											
					                             ...
					                             INNER JOIN user_master AS UM ON UM.user_id = PC.user_id  									
					                             	WHERE PS.payment_status='0' AND (PS.version = 'mobile_stripe') AND                                           
			...
			?>							
					...
					<tr><?php if($noOfPage > 1) { ?>
						...
						<a href="report_stripe_Payment_mobile.php?pageNo=1&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=<?=$tab?>">
						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>
						<a href="report_stripe_Payment_mobile.php?pageNo=<?=$prev?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=<?=$tab?>">
						...
						<select name="pageNo" class="paragraph"  onChange="form.submit()">
							...
							?>
							 ...
							 of <?=$noOfPage?>]
							 <a href="report_stripe_Payment_mobile.php?pageNo=<?=$next?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=<?=$tab?>">
							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>
							<a href="report_stripe_Payment_mobile.php?pageNo=<?=$noOfPage?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=<?=$tab?>">
...
//~ ) AND PC.status='REFUND'  $whereSql $joinquery GROUP BY PS.payment_currency order by PC.id desc ";			
  $amountQuery	=	"SELECT ( PS.payment_amount - PC.balance ) AS Total, PS.payment_currency,PC.balance,PC.join_date,UM.* FROM  		 		
 											...
 											INNER JOIN payment AS PS ON PC.payment_id = PS.payment_id
 											INNER JOIN user_master AS UM ON UM.user_id = PC.user_id WHERE                    	
 																PS.payment_status='1' AND (PS.version = 'mobile_stripe'
