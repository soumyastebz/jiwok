	...
	*/
	...
	elseif($tab == 4)
	{
		if($page_name!= "")
		{
					...
					$query		=	"SELECT count(*) AS total_condition	FROM payment As PS INNER JOIN user_master AS UM ON UM.user_id = PS.payment_userid  left join brand_user b on b.user_id=UM.user_id left join brand_master c on c.brand_master_id=b.brand_master_id WHERE UM.user_status !=0 AND PS. payment_status	=1  $whereSql $joinquery ";  
					        		
		}
		else
		{
		$query		=	"SELECT count(*) AS total_condition	FROM payment_paybox AS PP INNER JOIN user_master AS UM 						ON UM.user_id = PP.user_id left join brand_user b on b.user_id=UM.user_id left join           				brand_master c on c.brand_master_id=b.brand_master_id WHERE UM.user_status !=0 AND               			PP.status='ACTIVE'  $whereSql $joinquery";
		}
	}
	elseif($tab == 5)
	{
	...
	else
	{
		...
		$query		=	"SELECT count(*) AS total_condition	FROM payment AS PS LEFT JOIN user_master AS UM ON          				UM.user_id = PS.payment_userid left join brand_user b on b.user_id=UM.user_id left join 		 						brand_master c on c.brand_master_id=b.brand_master_id WHERE PS.payment_status='1' AND       					PS.version='New' $whereSql $joinquery";
	}
	...
	if($total_condition['total_condition']>0){
		...
		if($tab == 2)
		{ 
		$sql = "SELECT UM.user_fname, UM.user_lname,UM.user_email,PC.* FROM payment_transactions AS PC INNER JOIN             	payment AS PS ON PC.payment_id = PS.payment_id INNER JOIN user_master AS UM ON UM.user_id =                PC.user_id left join brand_user b on b.user_id=UM.user_id left join brand_master c on                	c.brand_master_id=b.brand_master_id WHERE PS.payment_status='0' AND PS.version='New' AND PC.status 				='CANCELLED' $whereSql $joinquery";
		}
		elseif($tab == 3)
		{
		$sql = "SELECT UM.user_fname, UM.user_lname,UM.user_email,PC.* 					
					...
					WHERE PS.payment_status='1' AND PS.version='New' AND PC.status='REFUND'
					$whereSql $joinquery
					";
		}
		elseif($tab == 4)
		...
		else
		{
			...
			$sql = "SELECT UM.user_fname, UM.user_lname,UM.user_email,PS.payment_amount,PS.payment_currency,PS.payment_expdate,UM.user_id, PC.date AS payment_date,PC.trans_refrns_id 					
					...
					left join brand_user b on b.user_id=UM.user_id 
					left join brand_master c on c.brand_master_id=b.brand_master_id
					WHERE PS.payment_status='1' AND PS.version='New' AND PC.status = 'PAID' $whereSql $joinquery
					";	
		}
	...
	//$sql = $sqlStart.$fromSql.$whereSql." GROUP BY user_master.user_id ORDER BY user_master.user_doj DESC";
		...
		if($tab == 2)
		{
			$paging_query	= "SELECT count(PC.user_id) 
					...
					left join brand_user b on b.user_id=UM.user_id 
					left join brand_master c on c.brand_master_id=b.brand_master_id
					WHERE PS.payment_status='0' AND PS.version='New' AND PC.status='CANCELLED'  
					$whereSql $joinquery";
		}
		elseif($tab == 3)
		{
		...
		else
		{
			$paging_query	= "SELECT count(PS.payment_userid) 
					...
					left join brand_user b on b.user_id=UM.user_id 
					left join brand_master c on c.brand_master_id=b.brand_master_id
					WHERE PS.payment_status='1' AND PS.version='New'  
					$whereSql $joinquery";
		}
...
<select name="toY">
  <?
	...
	<ul>		
        ...
        <li><a href="report_newPayment.php?pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=5"><span>Unsubscribed </span></a></li><?php } ?>
