...
*****************************************************************************/
	...
	*/
	...
	$sqlStart = "SELECT user_master.user_id,user_master.user_fname,user_master.user_lname,discount_users.start_date AS payDate,  discount_users.discount_code AS discCode, discount_users.payment_status AS payStatus, COUNT(discount_users.user_id) AS count";
	$fromSql  = " FROM user_master,discount_users ";
	$whereSql = " WHERE user_master.user_id = discount_users.user_id ";
	//Total num of jiwok users
	$sqlTotNum = "SELECT DISTINCT (user_master.user_id) AS cnt from user_master,discount_users WHERE user_master.user_id = discount_users.user_id  ".$joinquery;
	...
	*/
	...
	if($_POST['daterange'] == 1){
		...
		switch($_POST['dropdown1']){
			case 'today':
				$whereSql .= " AND discount_users.start_date = '".$today."'";
			...
			case 'yest':
				...
				$whereSql .= " AND discount_users.start_date = '".$yesterday."'";
			...
			case 'last7':
				...
				$whereSql .= " AND discount_users.start_date BETWEEN '".$requiredDate."' AND '".$today."'";
			...
			case 'thismonth':	
				...
				$whereSql 	 .= " AND MONTH(discount_users.start_date) = '".$thisMonth."'  AND YEAR(discount_users.start_date) = '".$thisYear."'";
			...
			case 'lastmonth':	
				...
				$whereSql 	 .= " AND MONTH(discount_users.start_date) = '".$lasMonth."'  AND YEAR(discount_users.start_date) = '".$lasYear."'";
				...
				$whereSql .= " AND YEAR(discount_users.start_date) = '".$lasYear."'";
		...
		if(count($errorMsg) == 0){
			if($startDate != $endDate){
				$whereSql 	 .= " AND discount_users.start_date BETWEEN '".$startDate."' AND '".$endDate."'";
				
			}else{ // Then we don't need a BETWEEN clause :)
				$whereSql .= " AND discount_users.start_date = '".$startDate."'";
	...
	if($_POST['report'] == 'disc' || $_POST['report'] == 'reff'){
	    if($_POST['report'] == 'disc')
		   $chkCondition = 'DISC';
		if($_POST['report'] == 'reff')
		  $chkCondition = 'REFF';
		
		 $whereSql .= " AND discount_users.discount_type = '".$chkCondition."'"; 
		 $sqlTotNum.=" AND discount_users.discount_type = '".$chkCondition."'";
		}
...
////////////////////////////////////Pagination ends here/////////////////////////////////////////
 ...
 if($totalRecs > 0){
	...
	$result=$objDb->_getList($sql);
		...
		$sql1	=	"SELECT  discount_users.user_id,discount_users.discount_code, discount_users.start_date, discount_users.payment_status FROM discount_users WHERE discount_users.user_id IN ($user_ids) ";
              ...
              <tr> 
			   ...
			   <form name="reportFrm" action="#" method="post">
				  ...
				  <tr>
				    ...
				    <td width="117" height="27" align="center" valign="bottom" class="sectionHeading"><a href="excel_discount.php?inf=<? echo base64_encode($sql);?>"><img src="../images/sports/english/download.gif" style="float:right" border="0"></a></td>
				  </tr>
  ...
  <?
				  ...
				  <tr style="padding-top:10px;">
					...
					<select id="report" name="report" onChange="change();">
					   ...
					   <option value="disc" <? if($_REQUEST['report']=='disc') echo 'selected="selected"'?>>Discount code</option>
					   <option value="reff" <? if($_REQUEST['report']=='reff') echo 'selected="selected"'?>>Referral code</option>
				  ...
				  <TABLE class="listTableStyle" cellSpacing=1 cellPadding=2 width="553">
                    <TBODY>
                      <TR class="tableHeaderColor">
                        ...
                        <TD width="23%" align="center" >Member Name </TD>
						<TD width="29%" align="center" >Code Used </TD>
                        <TD width="24%" align="center" >Payment Status  </TD>
                        <TD width="20%" align="center" >Paid Date </TD>
                      </TR>
                      <?php if($errMsg != ""){?>
                      ...
                      <tr class="listingTable">
                        <TD align="center" colspan="5" class="tblbackgnd"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                            ...
                            <tr  <? if(($key%2) ==1){?> bgcolor="#FFFFFF" <? } ?> >
                              ...
                              <TD width="21%" height="19" align="left" style="padding-left:10px;"><a href="javascript:;" onMouseOver="myFunction('<?=$val['user_id']?>')" onClick="return overlib(INARRAY, <?=$val['user_id']?>, myFunction('<?=$val['user_id']?>'), CAPTION, '<? echo addslashes(htmlspecialchars($objGen->_output($val['user_fname'])))." ".addslashes(htmlspecialchars($objGen->_output($val['user_lname']))).' - '.$val['count'];?>');" ><? echo $objGen->_output($val['user_fname'])." ".$objGen->_output($val['user_lname']);?></a></TD>
                               <TD width="29%" height="19" align="center" style="padding-left:10px;"><? echo $val['discCode']?></TD>
							  <TD width="23%" height="19" align="center" style="padding-left:10px;"><? echo $val['payStatus']?></TD>
                              <TD width="21%" height="19" align="center" style="padding-left:10px;"><? echo $val['payDate']?></TD>
                            </tr>
				...
				<table cellspacing=0 cellpadding=0 width=553 border=0 class="topColor">
					...
					<tr><?php if($noOfPage > 1) { ?>
						...
						<a href="report_discount.php?pageNo=1&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>">
						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>
						<a href="report_discount.php?pageNo=<?=$prev?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>">
						...
						<select name="pageNo" class="paragraph"  onChange="form.submit()">
							...
							?>
							 ...
							 of <?=$noOfPage?>]
							 <a href="report_discount.php?pageNo=<?=$next?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>">
							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>
							<a href="report_discount.php?pageNo=<?=$noOfPage?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>">
    ...
    <TD vAlign=top align=left class="topBarColor" colspan="3">&nbsp;</TD>
		...
		?><div id="<?=$result[$i]['user_id']?>" class="hidden_div" >
			<div class="popup_box">
				<table width="100%">
					<tr>
						<td width="34%" class="boldC">Discount Code</td>
						<td width="37%" class="boldC">Applied Date</td>
						<td width="29%" class="boldC">Payment Status</td>
					</tr>
				<?php
				for($j=0, $max2=sizeof($result[$i]['user_id']); $j<$max2; $j++){
					for($k=0, $max3=sizeof($programs[$result[$i]['user_id']]); $k<$max3; $k++){
				?><tr>
					<td width="34%"><?=$programs[$result[$i]['user_id']][$k]['discount_code']?></td>
					<td width="37%"><?=$programs[$result[$i]['user_id']][$k]['start_date']?></td>
					<td width="29%"><?=$programs[$result[$i]['user_id']][$k]['payment_status']?></td>
				</tr>
				<?
					}
				} 
