...
//	error_reporting(0);
	...
	*/
	...
	$query		=	"SELECT count(*) AS total_condition	FROM payment AS PS LEFT JOIN user_master AS UM ON 
		                 ...
		                 (PS.version = 'IAP')  $whereSql $joinquery";
	...
	if($total_condition['total_condition']>0){
	   ...
	   $sql = "SELECT UM.user_fname, UM.user_lname,UM.user_email,PS.payment_amount,PS.payment_currency,
			         PS.payment_expdate,UM.user_id,PS.payment_date AS payment_date				
					FROM payment AS PS																				
					...
					WHERE PS.payment_status='1' AND (PS.version = 'IAP') $whereSql $joinquery
					";	
		}
	...
	//$sql = $sqlStart.$fromSql.$whereSql." GROUP BY user_master.user_id ORDER BY user_master.user_doj DESC";
		...
		{   
			$paging_query	= "SELECT count(PS.payment_userid) 
					...
					left join brand_user b on b.user_id=UM.user_id 
					left join brand_master c on c.brand_master_id=b.brand_master_id
					WHERE PS.payment_status='1' AND (PS.version = 'IAP')  
					$whereSql $joinquery";
		}
...
<select name="toY">
  <?
	...
	<ul>		
        <li><a href="report_IAP_Payment.php?pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=1"><span>Payments</span></a></li>
		  ...
		  <?php if($tab == 1){
			...
			?>							
					...
					<tr><?php if($noOfPage > 1) { ?>
						...
						<a href="report_IAP_Payment.php?pageNo=1&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=<?=$tab?>">
						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>
						<a href="report_IAP_Payment.php?pageNo=<?=$prev?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=<?=$tab?>">
						...
						<select name="pageNo" class="paragraph"  onChange="form.submit()">
							...
							?>
							 ...
							 of <?=$noOfPage?>]
							 <a href="report_IAP_Payment.php?pageNo=<?=$next?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=<?=$tab?>">
							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>
							<a href="report_IAP_Payment.php?pageNo=<?=$noOfPage?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=<?=$tab?>">
