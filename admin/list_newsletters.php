...
/**************************************************************************** 
   ...
   DESCRIPTION::::>>>>
   This  code userd to list the all Newsletter.
   Admin can add/edit/delete the Newsletter .. 
*****************************************************************************/
	...
	*/
	...
	if($_REQUEST['field']){
		$field = "newsletter_manager.".$_REQUEST['field'];
		$type = $_REQUEST['type'];
	}else{
		$field = "newsletter_id";
		$type = "ASC";
	...
	if(trim($_REQUEST['keyword'])){
		...
		$errMsg = "Special characters are not allowed";
			...
			$searchQuery	=	" AND manager_title like '%".$cleanData."%'";		}
	}
	...
	if($_REQUEST['action'] == "delete"){
		$id		 = $_REQUEST['newsletterId'];
		$result	 = $objNewsletter->_deleteNewsletter($id);
		$confMsg = "Successfully Deleted";
	}	
	
	$totalRecs = $objNewsletter->_getTotalCount($searchQuery,$lanId);
	if($totalRecs <= 0)
		$errMsg = "No Records";
	...
	if($_REQUEST['pageNo']){
		...
		$result =  $objNewsletter->_showPage($totalRecs,$_REQUEST['pageNo'],$_REQUEST['maxrows'],$field,$type,$searchQuery);
	}
	else{
	...
	if($totalRecs <= $_REQUEST['pageNo']*$_REQUEST['maxrows'])
	{
		...
		$endNo = $totalRecs;
		$displayString = "Viewing $startNo to $endNo of $endNo newsletters";
	...
	else
	{
		...
		$displayString = "Viewing $startNo to $endNo of $totalRecs newsletter's";
...
<tr> 
              ...
              <tr> 
			   ...
			   <form name="frmnewsletters" action="list_newsletters.php" method="post">
				  ...
				  <tr>
					...
					<TD align="left">
						...
						<?  if($objGen->_output($_REQUEST['keyword'])){ ?>
							<td valign="middle" width="50"><a href="list_newsletters.php?maxrows=<?=$_REQUEST['maxrows']?>"><img src="images/list.gif" alt="Listing Record">&nbsp;List </a></td>
						<? }else{ ?>
							<td valign="middle" width="50" class="noneAnchor"><img src="images/list.gif" alt="Listing Record">&nbsp;List </td>
						<? } ?>
						<td valign="middle" width="50"><a href="addedit_newsletters.php?action=add&pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>"><img src="images/add.gif" border="0" alt="Add Record">&nbsp;Add   </a></td>
                     ...
                     <table  cellspacing=0 cellpadding=0 width="553" border=0 class="topColor">
                      <tbody>
	    			    	<tr> 
					   <td width="192" valign=top class="paragraph2"><?=$displayString?>
					   </td>
					   <td width="131" valign=top class="paragraph2">Language:
					   ...
					   <td width="230" align=right class="paragraph2"><?=$heading;?> per page: 
				  ...
				  <TABLE class="listTableStyle" cellSpacing=1 cellPadding=2 width="553">
				   <TBODY> 
					   <TR class="tableHeaderColor">
						...
						<a href="list_newsletters.php?field=manager_title&type=asc&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>"><img src="images/up.gif" border="0" alt="Ascending Sort">&nbsp;</a>
						<a href="list_newsletters.php?field=manager_title&type=desc&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>"><img src="images/down.gif" border="0" alt="Descending Sort"></a>
					 ...
					 <?php }
						...
						?>
							...
							<TD><?=stripslashes($objGen->_output($row['manager_title']))?></TD>
							
							<TD align="center"><?php if($row['newsletter_status'] == 1) echo "<img src=\"images/y.gif\" width=\"14\" height=\"14\">"; else echo "<img src=\"images/n.gif\" width=\"14\" height=\"14\">";?></TD>
							<TD align="center">
								<a href = "addedit_newsletters.php?newsletterId=<?=$row['newsletter_id']?>&pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&langId=<?=$lanId?>&type=<?=$_REQUEST['type']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>&action=edit" class="smallLink">Edit</a>&nbsp;
								|
								<a href = "list_newsletters.php?newsletterId=<?=$row['newsletter_id']?>&pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>&action=delete&langId=<?=$lanId?>" class="smallLink" onClick="return confirm('Are you sure that you want to delete the selected Record? If yes click Ok, if not click Cancel.')">Delete</a></TD> 
				...
				<table cellspacing=0 cellpadding=0 width=553 border=0 class="topColor">
					...
					<tr>
						...
						<a href="list_newsletters.php?pageNo=1&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>">
						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>
						<a href="list_newsletters.php?pageNo=<?=$prev?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>">
						...
						<select name="pageNo" class="paragraph"  onChange="form.submit()">
							...
							?>
							 ...
							 of <?=$noOfPage?>]
							 <a href="list_newsletters.php?pageNo=<?=$next?>&langId=<?=$lanId?>&type=<?=$_REQUEST['type']?>&field=<?=$_REQUEST['field']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>">
							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>
							<a href="list_newsletters.php?pageNo=<?=$noOfPage?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>">
