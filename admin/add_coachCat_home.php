...
*****************************************************************************/
	...
	*/
	...
	$errorMsg	=	array();
	if($_POST['addnew']){reset($languageArray);
		...
		if(count($errorMsg)==0)	{
			reset($languageArray);
			while(list($key,$value) = each($languageArray)){
					$query = "SELECT  count(*) as dataCount FROM coach_category_manager_home WHERE coach_categorymaster_id =".$_REQUEST['coachCatId']." AND language_id=".$key." and common_name='".$_REQUEST['coach_name_2']."'";
...
<tr> 
              ...
              <tr> 
				...
				<form name="serviceform" action="add_coachCat_home.php" method="post" onSubmit="return formChecking()">
...
-->
					...
					<tr>
						<td colspan="2" align="center">
							...
							<input type="submit" name="addnew" value="&nbsp;Add&nbsp;"></td>
					</tr>
					<?php	}	?>
				    </tbody>
			 	  </table>
