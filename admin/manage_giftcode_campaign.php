...
//	error_reporting(0);
	...
	*/
	...
	$heading = "Gift Code Campaign Management";
	...
	if($_REQUEST['field']){
		...
		$field = "gift_pay_campaign.".$_REQUEST['field'];
 ...
 $totalRecs = $objCamp->_getTotalCount($searchQuery);
...

	{
		...
		$displayString = "Viewing $startNo to $endNo of $endNo campaigns";
...

	{
		...
		$displayString = "Viewing $startNo to $endNo of $totalRecs campaigns";
...
<tr> 
              ...
              <tr> 
			   ...
			   <form name="frmcamp" action="manage_giftcode_campaign.php" method="post">
				  ...
				  <tr>
					...
					<TD align="left">
						...
						<?  if($objGen->_output($_REQUEST['keyword'])){ ?>
							...
							<td valign="middle" width="50"><a href="manage_giftcode_campaign.php?maxrows=<?=$_REQUEST['maxrows']?>"><img src="images/list.gif" alt="Listing Record">&nbsp;List </a></td>
						...
						<td valign="middle" width="50"><a href="addedit_gift_campaign.php?action=add&pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>"><img src="images/add.gif" border="0" alt="Add Record">&nbsp;Add   </a></td>
				  ...
				  <TABLE class="listTableStyle" cellSpacing=1 cellPadding=2 width="553">
				   ...
				   <TBODY> 
					   ...
					   <TR class="tableHeaderColor">
						...
						<a href="manage_giftcode_campaign.php?field=camp_name&type=asc&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>"><img src="images/up.gif" border="0" alt="Ascending Sort">&nbsp;</a>

						<a href="manage_giftcode_campaign.php?field=camp_name&type=desc&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>"><img src="images/down.gif" border="0" alt="Descending Sort"></a>
						...
						<TD width="15%" align="center" >Plan Period</TD>

						<TD width="13%" align="center" >Created Date</TD>
					 ...
					 <?php }
						...
						?>
							...
							<TD><?=stripslashes(stripslashes(stripslashes($row['camp_discount'])));?></TD>

							<TD align="center"><?=stripslashes(stripslashes(stripslashes($row['camp_price'])));?></TD>

							<TD align="center"><?=stripslashes(stripslashes(stripslashes($row['valid_months'])));?></TD>
                            <TD align="center"><?=stripslashes(stripslashes(stripslashes($row['created_date'])));?></TD>
							...
							<TD align="center">
								...
								<a href = "addedit_gift_campaign.php?id=<?=$row['id']?>&pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&type=<?=$_REQUEST['type']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>&action=edit" class="smallLink">Edit</a>&nbsp;

								|

								<a href = "manage_giftcode_campaign.php?id=<?=$row['id']?>&pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>&action=delete" class="smallLink" onClick="return confirm('Are you sure that you want to delete the selected Record? If yes click Ok, if not click Cancel.')">Delete</a></TD> 
				...
				<table cellspacing=0 cellpadding=0 width=553 border=0 class="topColor">
					...
					<tr>
						...
						<a href="manage_giftcode_campaign.php?pageNo=1&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>">

						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>

						<a href="manage_giftcode_campaign.php?pageNo=<?=$prev?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>">
						...
						<select name="pageNo" class="paragraph"  onChange="form.submit()">
							...
							?>
							 ...
							 <a href="manage_giftcode_campaign.php?pageNo=<?=$next?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&field=<?=$_REQUEST['field']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>">

							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>

							<a href="manage_giftcode_campaign.php?pageNo=<?=$noOfPage?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>">
