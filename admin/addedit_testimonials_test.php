...
*****************************************************************************/
	...
	include("../includes/classes/class.testimonials_test.php");
	...
	*/
	...
	if($_POST['add']||$_POST['update']){
		...
		while(list($key,$value) = each($languageArray)){
				if(trim($_POST['testimonial_desc_'.$key])=='')
	...
	if($_POST['add']){
		...
		// Insert Data
			...
			reset($languageArray);
			while(list($key,$value) = each($languageArray)){
				$desc	= $_POST['testimonial_desc_'.$key];
				$elmts	= array("testimonial_desc" => $desc,"master_id" => $nextId,"lang_id" => $key);
				
				$result =  $objTestimon->_insertTestimonial($elmts,count($languageArray));
			}
			header("Location:list_testimonials_test.php?status=success_add&langId=".$_REQUEST['langId']."&pageNo=".$_REQUEST['pageNo']."&maxrows=".$_REQUEST['maxrows']."&type=".$_REQUEST['type']."&field=".$_REQUEST['field']."&keyword=".$_REQUEST['keyword']);
		}
	}
	...
	if($_POST['update']){
		...
		if(count($errorMsg)==0)	{
			...
			reset($languageArray);
			while(list($key,$value) = each($languageArray)){
					 $query = "SELECT  count(*) as dataCount FROM testimonial WHERE master_id =".$masterId." AND lang_id=".$key.""; 
					...
					if(DB::isError($result)) 
						...
						if($coundRecs >= 1){
							...
							$result = $objTestimon->_updateTestimoni($masterId,$key,$elmts);
						}
						else
						{
						    $desc	= $_POST['testimonial_desc_'.$key];
							$result = $objTestimon->_insertOneTestimoni($masterId,$key,$desc);
						}
				
			}
			header("Location:list_testimonials_test.php?status=success_update&langId=".$_REQUEST['langId']."&pageNo=".$_REQUEST['pageNo']."&maxrows=".$_REQUEST['maxrows']."&type=".$_REQUEST['type']."&field=".$_REQUEST['field']."&keyword=".stripslashes(stripslashes($_REQUEST['keyword'])));
...
<tr> 
              ...
              <tr> 
				...
				<form name="faqform" action="addedit_testimonials_test.php" method="post" onSubmit="return formChecking()" enctype="multipart/form-data">
                            ...
                            <TD valign="top">
								  ...
								  <tr>
									...
									<TD align="left">
										...
										<table height="50" class="topActions"><tr><td valign="middle" width="50"><a href="list_testimonials_test.php?pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=stripslashes(stripslashes($_REQUEST['keyword']))?>"><img src="images/list.gif" border="0" alt="Listing Record">&nbsp;List   </a></td>
				  ...
				  <TABLE class="paragraph2" cellSpacing=0 cellPadding=2 width="100%">
				   <TBODY> 
					...
					?> 
					 <tr height="30px">
						<td width="30%" align="right"> User Name<?php echo REQUIRED;?>:&nbsp;</td>
						<td>
						<input type="text" name="user_name" id="user_name" value="<?=$objGen->_output($_POST['user_name']);?>"/>
						</td>
					</tr>
					<tr height="30px">
						<td width="30%" align="right"> Testimonial for:&nbsp;</td>
						<td>
					...
					<tr height="30px">
					<td width="30%" align="right"> Brand Name:&nbsp;</td>
						<td>
					...
					<tr height="30px">
						<td width="30%" align="right"> User Image:&nbsp;<br/>
				   ...
				   <table width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					<tr>
						...
						<td><textarea name="testimonial_desc_<?=$key?>" cols="50" rows="5" ><?=$objGen->_output($_POST['testimonial_desc_'.$key])?></textarea>	</td>
					</tr>
					  </table>
					  </fieldset>
					<? 
