	...
	*/
	...
	if($_POST['add']||$_POST['update']){
		...
		if($_POST['update'])	{
			...
			while(list($key,$value) = each($languageArray)){
				...
				$check = false;
				$check	= $objService->_isServiceExists($objGen->_clean_data($_REQUEST['service_content'.$key]),$_REQUEST['serviceId']);
				if($check) 
	...
	if($_POST['add']){
		...
		if(count($errorMsg)==0)	{
			...
			header("Location:list_services.php?status=success_add&langId=".$_REQUEST['langId']."&pageNo=".$_REQUEST['pageNo']."&maxrows=".$_REQUEST['maxrows']."&type=".$_REQUEST['type']."&field=".$_REQUEST['field']."&keyword=".$_REQUEST['keyword']);
		}
	}
			...
			while(list($key,$value) = each($languageArray)){
					...
					if(DB::isError($result)) 
						...
						if($coundRecs >= 1){
							...
							$result = $objService->_updateService($_REQUEST['serviceId'],$key,$elmts);
						}
						else
						{
						    $title	= $_POST['service_content_'.$key];
							$body	= $_POST['service_description_'.$key];
							$result = $objService->_insertOneService($_REQUEST['serviceId'],$key,$title,$body);
						}
				
			}
			//print_r($title);
			
			header("Location:list_services.php?status=success_update&langId=".$_REQUEST['langId']."&pageNo=".$_REQUEST['pageNo']."&maxrows=".$_REQUEST['maxrows']."&type=".$_REQUEST['type']."&field=".$_REQUEST['field']."&keyword=".$_REQUEST['keyword']);
	...
	if($_REQUEST['serviceId'] and count($errorMsg)==0){
		...
		$result = $objService->_getAllById($_REQUEST['serviceId']);
...
<tr> 
              ...
              <tr> 
				...
				<form name="serviceform" action="addedit_services.php" method="post" onSubmit="return formChecking()">
                            ...
                            <TD valign="top">
								  ...
								  <tr>
									...
									<TD align="left">
										...
										<table height="50" class="topActions"><tr><td valign="middle" width="50"><a href="list_services.php?pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$_REQUEST['keyword']?>"><img src="images/list.gif" border="0" alt="Listing Record">&nbsp;List   </a></td>
