<? 
 //for generating the excell sheet for the report
	...
	$worksheet->write(0, 1, " Subscription Report  ", $format_title);
	
	$worksheet->write(1,0, 'Index', $format_bold);
	$worksheet->write(1,1, 'Name', $format_bold);
    $worksheet->write(1,2, 'Subscriptions No', $format_bold);   
	$worksheet->write(1,3, 'Percentage', $format_bold);
	...
	if(count($memberList)>0){
        ...
        $worksheet->write($row,2,$val['count']);
        $worksheet->write($row,3, $val['percent']);
		
	    }
		}
	}
	
	$workbook->send('Subscription_report_jiwok_'.date('d_m_Y').'.xls');
	// Let's send the file
	$workbook->close();
	}
