...
//	echo $searchQuery;
	...
	if($page_name!= "")
	{
			...
			$query = "SELECT count(*) as max FROM feedback,program_detail,user_master WHERE feedback.program_id = program_detail.program_master_id AND feedback.user_id = user_master.user_id AND program_detail.language_id= 1 AND feedback.lang_id=5 ";
			if($searchQuery)
	...
	{
		$totalRecs = $objFeedback->_getTotalCount($searchQuery);
	}
	...
	if($_REQUEST['pageNo']){
		...
		$result =  $objFeedback->_showPage($totalRecs,$_REQUEST['pageNo'],$_REQUEST['maxrows'],$field,$type,$searchQuery,$page_name);
	}
	else{
