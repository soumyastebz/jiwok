	...
	*/
	...
	if(trim($_REQUEST['keyword'])){
		...
		$errMsg = "Special characters are not allowed";
				 ...
				 $searchQuery	= " and (name like '%".$cleanData."%')";}
	}	
	...
	if($_REQUEST['action'] == "delete"){
		...
		$result	 = $objseo->_deleteseo($id);
		$confMsg = "Successfully Deleted";
	}	
	$totalRecs = $objseo->_getTotalCount($searchQuery,$lanId);
	...
	if($_REQUEST['pageNo']){
		...
		$result =  $objseo->_showPage($totalRecs,$_REQUEST['pageNo'],$_REQUEST['maxrows'],$field,$type,$searchQuery);
	}
	else{
                ...
                <TD class=smalltext width="98%" valign="top"><table width="75%" border="0" align="center" cellpadding="0" cellspacing="0">
                    ...
                    <tr>
                      ...
                      <td valign="top"><TABLE cellSpacing=0 cellPadding=0 border=0 align="center">
                          <TR>
                            <TD vAlign=top width=564 bgColor=white><form name="frmhomepage" action="list_seo.php" method="post">
                                  ...
                                  <TR>
                                    <TD align="left"><table height="50"  width="100%"class="topActions">
                                        <tr>
                                          ...
                                          <td valign="middle" width="50"><a href="list_seo.php?maxrows=<?=$_REQUEST['maxrows']?>"><img src="images/list.gif" alt="Listing Record">&nbsp;List </a></td>
                                          <? }else{ ?>
                                          <td valign="middle" width="50" class="noneAnchor"><img src="images/list.gif" alt="Listing Record">&nbsp;List </td>
                                          <? } ?>
                                          <td valign="middle" width="50"><a href="addedit_seo.php?action=add&pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$objGen->_output(stripslashes($_REQUEST['keyword']))?>"><img src="images/add.gif" border="0" alt="Add Record">&nbsp;Add </a></td>
                                ...
                                <TABLE class="listTableStyle" cellSpacing=1 cellPadding=2 width="553">
                                  <TBODY>
                                    <TR class="tableHeaderColor">
                                      ...
                                      <TD width="40%" >Page&nbsp;name <a href="list_seo.php?field=name&type=asc&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$objGen->_output(stripslashes($_REQUEST['keyword']))?>"><img src="images/up.gif" border="0" alt="Ascending Sort">&nbsp;</a> <a href="list_seo.php?field=name&type=desc&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$objGen->_output(stripslashes($_REQUEST['keyword']))?>"><img src="images/down.gif" border="0" alt="Descending Sort"></a> </TD>
									  
                                      <TD width="15%" align="center" >Title<a href="list_seo.php?field=meta_title&type=asc&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$objGen->_output(stripslashes($_REQUEST['keyword']))?>"><img src="images/up.gif" border="0" alt="Ascending Sort">&nbsp;</a> <a href="list_seo.php?field=meta_title&type=desc&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$objGen->_output(stripslashes($_REQUEST['keyword']))?>"><img src="images/down.gif" border="0" alt="Descending Sort"></a></TD>
						...
						?>
                                    <tr class="listingTable">
                                      ...
                                      <TD><?=stripslashes(stripslashes(stripslashes($row['name'])));?></TD>
									  ...
									  <TD><?=stripslashes(stripslashes(stripslashes($row['meta_title'])));?></TD>
                                      <TD align="center"><a href = "addedit_seo.php?masterId=<?=$row['master_id']?>&pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&type=<?=$_REQUEST['type']?>&keyword=<?=$objGen->_output(stripslashes($_REQUEST['keyword']))?>&action=edit" class="smallLink">Edit</a>&nbsp;
                                        | <a href = "list_seo.php?masterId=<?=$row['master_id']?>&pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$objGen->_output(stripslashes($_REQUEST['keyword']))?>&action=delete" class="smallLink" onClick="return confirm('Are you sure that you want to delete the selected Record? If yes click Ok, if not click Cancel.')">Delete</a></TD>
                                    </tr>
                                    <?php
						$count++;
						}
						?>
                                ...
                                <table cellspacing=0 cellpadding=0 width=553 border=0 class="topColor">
                                  <tbody>
                                    <tr>
                                      <td align="left" colspan = "6" class="leftmenu"><a href="list_seo.php?pageNo=1&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output(stripslashes($_REQUEST['keyword']))?>"> <img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a> <a href="list_seo.php?pageNo=<?=$prev?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output(stripslashes($_REQUEST['keyword']))?>"> <img src="images/mail-arrowlg.gif" border=0 width="20" height="20" align="absmiddle" alt="Previous Page"></a>[Page
							...
							?>
                                        ...
                                        of
                                        <?=$noOfPage?>
                                        ] <a href="list_seo.php?pageNo=<?=$next?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&field=<?=$_REQUEST['field']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$objGen->_output(stripslashes($_REQUEST['keyword']))?>"> <img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a> <a href="list_seo.php?pageNo=<?=$noOfPage?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output(stripslashes($_REQUEST['keyword']))?>"> <img src="images/mail-arrowlastg.gif" border=0 width="20" height="20" align="absmiddle" alt="Last Page"></a> </td>
                                    </tr>
                                  </tbody>
                                </table>
                                <?php } ?>
