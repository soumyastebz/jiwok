...
*****************************************************************************/
	...
	*/
	...
	$fromSql  = " FROM user_master LEFT JOIN brand_user ON brand_user.user_id	=	user_master.user_id ";

	$whereSql = " WHERE user_master.user_type=1 ";

	//Total num of jiwok users

	$sqlTotNum = "SELECT count(*) as count from user_master WHERE user_master.user_type=1 ".$joinQuery;
	...
	if($_POST['user_brand']!="" ){
		...
		if($_POST['user_brand']	==	1)
			$whereSql .= " AND brand_user.brand_master_id IS NULL ";
		else
			$whereSql .= " AND brand_user.brand_master_id = ". $_POST['user_brand']." ";
	}
	...
	*/
	...
	$sql		    = $sqlStart.$fromSql.$whereSql.$joinQuery." GROUP BY user_master.user_id ORDER BY user_master.user_doj DESC";
	$sql_download	=	$sql;
	$sql_count		= 'SELECT COUNT(*) as count '.$fromSql.$whereSql.$joinQuery;
              ...
              <tr> 
			   ...
			   <form name="reportFrm" action="#" method="post">
				  ...
				  <tr>
				    ...
				    <td width="117" height="27" align="center" valign="bottom" class="sectionHeading"><a href="excel_mem.php?inf=<? echo base64_encode($sql_download);?>"><img src="../images/sports/english/download.gif" style="float:right" border="0"></a></td>
                      ...
                      <tr class="listingTable">
                        ...
                        <TD align="center" colspan="4" class="tblbackgnd"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                            ...
                            <tr  <? if(($key%2) ==1){?> bgcolor="#FFFFFF" <? } ?> >
                              ...
                              <TD width="32%" height="19" align="left" style="padding-left:10px;"><? echo $val['user_email']?></TD>

                              <TD width="25%" height="19" align="left" style="padding-left:10px;"><? echo $val['date']?></TD>
