...
/**************************************************************************** 
   ...
   DESCRIPTION::::>>>>
   To generate the report for the New stripe payment.
...
//	error_reporting(0);
	...
	*/
	...
	$heading        = "New Stripe Payment Report";
	...
	*/
	...
	else
	{
		...
		$query		=	"SELECT count(*) AS total_condition	FROM payment AS PS LEFT JOIN user_master AS UM ON          				UM.user_id = PS.payment_userid left join brand_user b on b.user_id=UM.user_id left join brand_master c on c.brand_master_id=b.brand_master_id WHERE PS.payment_status='1' AND      	(
PS.version = 'stripe'
OR PS.version = 'polishstripe'
	...
	if($total_condition['total_condition']>0){
		...
		elseif($tab == 4)
		{
			if($page_name!= "")
			{
				    $sql = "SELECT UM.user_fname,UM.user_lname,UM.user_email,UM.user_doj,PS.payment_status 
				    FROM payment As PS INNER JOIN user_master AS UM ON UM.user_id = PS.payment_userid left join brand_user b on b.user_id=UM.user_id 
					left join brand_master c on c.brand_master_id=b.brand_master_id
			...
			{      //gg
		     	   $sql = "SELECT UM.user_fname,UM.user_lname,UM.user_email,ST.join_date,ST.status					
		...
		elseif($tab == 5)
		{
					...
					$sql = "SELECT UM.user_fname, UM.user_lname,UM.user_email,ST.status 					
					FROM `stripe_transaction`  AS ST															
					INNER JOIN user_master AS UM ON UM.user_id = ST.user_id 
					left join brand_user b on b.user_id=UM.user_id 
					left join brand_master c on c.brand_master_id=b.brand_master_id
					WHERE UM.user_status !=0 AND ST.status	!=	'ACTIVE' $whereSql $joinquery";
...
AND PC.status = 'PAID' $whereSql $joinquery";
	...
	<ul>		
        ...
        <li><a href="report_stripe_Payment.php?pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=5"><span>Unsubscribed </span></a></li><?php } ?>
			...
			?>							
				  ...
				  if(count($result)>0)
				  {
					echo  $amountQuery	=	"SELECT sum( PS.payment_amount ) AS Total, PS.payment_currency
