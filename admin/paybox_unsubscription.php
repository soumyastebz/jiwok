<?php
	...
	$heading = "Unsubcribe from paybox";
	$errMesg = "";
	$confMsg = "";
	...
	$parObj 	=   new Contents('paybox_unsubscription.php');
	...
	if($_REQUEST['action'] == "unsubscribe" && $_REQUEST['id']!=""){
		   ...
		   if($res == '00000')
		   {
				...
				$sqlQry		    =	"SELECT user_id FROM payment_paybox where id='".$_REQUEST['id']."'";
				$resQry			=	$payObj->dbSelectOne($sqlQry);
				$userId			=	$resQry[user_id];
				$userdetails=$memObj->_getOneUser($resQry[user_id]);
				if($userdetails)
				{
	...
	$query	=	"SELECT count(*) as max,PP.*,UM.user_email FROM payment_paybox PP LEFT JOIN user_master UM on UM.user_id = PP.user_id";	
	...
	if($_REQUEST['pageNo']){
		...
		$result = $payObj->_showPageUnsubscribe($totalRecs,$_REQUEST['pageNo'],$_REQUEST['maxrows'],$field,$type,$searchQuery);
	}
	else{
...
<tr> 
              ...
              <tr> 
			   ...
			   <form name="frmadmin" action="paybox_unsubscription.php" method="post">
				  ...
				  <tr>
                       ...
                       <tbody>
						...
						?>
                         <tr class="listingTable">
                           ...
                           <td><?=$genObj->_output($row->brand);?></td>
                           <td align="center"><?=$genObj->_output($row->join_date);?></td>
                           <td align="center"><?php if($genObj->_output($row->status)	== 'ACTIVE') echo "<img src=\"images/y.gif\" width=\"14\" height=\"14\" title=\"Active\">"; elseif($genObj->_output($row->status)	== 'UNSUBSCRIBED') echo "<img src=\"images/n.gif\" width=\"14\" height=\"14\" title=\"Unsubscribed\">"; else echo "<img src=\"images/n.gif\" width=\"14\" height=\"14\" title=\"Expired\">";?></td>                           
                           <td align="center">&nbsp;<?php if($genObj->_output($row->status) == 'ACTIVE'){?><a href = "paybox_unsubscription.php?id=<?=$row->id?>&pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&langId=<?=$lanId?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>&action=unsubscribe" class="smallLink" onClick="return confirm('Are you sure that you want to Unsubscribe this user from paybox?.If yes click Ok, if not click Cancel.')"><strong>Unsubscribe</strong></a><?php }?></td>
                    ...
                    <?php if($noOfPage > 1) {?>
						...
						<a href="paybox_unsubscription.php?pageNo=1&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>
						<a href="paybox_unsubscription.php?pageNo=<?=$prev?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
						...
						<select name="pageNo" class="paragraph"  onChange="form.submit()">
							...
							?>
							 ...
							 of <?=$noOfPage?>]
							 <a href="paybox_unsubscription.php?pageNo=<?=$next?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&field=<?=$_REQUEST['field']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>
							<a href="paybox_unsubscription.php?pageNo=<?=$noOfPage?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
