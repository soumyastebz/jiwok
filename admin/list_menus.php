...
/**************************************************************************** 
   ...
   Admin can add/edit/delete the menus 
*****************************************************************************/
	...
	if($_REQUEST['action'] == "delete"){
		...
		$query = "DELETE FROM label_manager WHERE label_type='MENU' and labeltype_id=".$_REQUEST['menuId'];
		$result = $GLOBALS['db']->query($query);
		$query = "DELETE FROM menus WHERE menu_id=".$_REQUEST['menuId'];
		$result = $GLOBALS['db']->query($query);				
		$confMsg = "Successfully Deleted";
	}	
	
	$query = "SELECT count(*) as max FROM menus,label_manager WHERE menus.menumaster_id = ".$_REQUEST['masterId']." and label_manager.label_type='MENU' and menus.menu_id =label_manager.labeltype_id and label_manager.language_id=".$_REQUEST['langId'];
	...
	if($_REQUEST['pageNo']){
		...
		$result = $pageObj->_showMenuPage($_REQUEST['langId'],$totalRecs,$_REQUEST['masterId'],$_REQUEST['pageNo'],$_REQUEST['maxrows'],$field,$type,$searchQuery);
	}
	else{
...
<tr> 
              ...
              <tr> 
			   ...
			   <form name="frmadmin" action="list_menus.php" method="post">
				  ...
				  <tr>
					...
					<TD align="left">
						...
						<? if($genObj->_output($_REQUEST['keyword'])){ ?>
							<td valign="middle" width="50" ><a href="list_menus.php?masterId=<?=$_REQUEST['masterId']?>&heading=<?=$_REQUEST['heading']?>&langId=<?=$_REQUEST['langId']?>&maxrows=<?=$_REQUEST['maxrows']?>"><img src="images/list.gif" alt="Listing Record">&nbsp;List </a></td>
						<? }else{ ?>
							<td valign="middle" width="50" class="noneAnchor"><img src="images/list.gif" alt="Listing Record">&nbsp;List </td>
						<? } ?>
						<td valign="middle" width="50"><a href="addedit_menu.php?action=add&masterId=<?=$_REQUEST['masterId']?>&heading=<?=$_REQUEST['heading']?>&langId=<?=$_REQUEST['langId']?>&pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&type=<?=$_REQUEST['type']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>"><img src="images/add.gif" border="0" alt="Add Record">&nbsp;Add   </a></td>
<td ><a href="list_menumaster.php"><img src="images/add.gif" border="0" alt="Lust Menus">&nbsp;List Menu </a></td>
                     ...
                     <table  cellspacing=0 cellpadding=0 width="553" border=0 class="topColor">
                      <tbody>
	    			    	...
	    			    	<tr> 
					   <td class="paragraph2" valign=top><?=$genObj->_output($genObj->_output($displayString));?> <?=str_repeat('&nbsp',7); ?> Language
				  ...
				  <TABLE class="listTableStyle" cellSpacing=1 cellPadding=2 width="553">
				   <TBODY> 
					   <TR class="tableHeaderColor">
						...
						<a href="list_menus.php?field=label_name&type=asc&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&masterId=<?=$_REQUEST['masterId']?>&langId=<?=$_REQUEST['langId']?>&heading=<?=$_REQUEST['heading']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>"><img src="images/up.gif" border="0" alt="Ascending Sort">&nbsp;</a>
						<a href="list_menus.php?field=label_name&type=desc&maxrows=<?=$_REQUEST['maxrows']?>&langId=<?=$_REQUEST['langId']?>&pageNo=<?=$_REQUEST['pageNo']?>&masterId=<?=$_REQUEST['masterId']?>&heading=<?=$_REQUEST['heading']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>"><img src="images/down.gif" border="0" alt="Descending Sort"></a>
					 ...
					 <?php }
						...
						?>
							...
							<TD><?=$genObj->_output($row->label_name);?></TD>
							
							<TD align="center"><?php if($row->menu_status == 1) echo "<img src=\"images/y.gif\" width=\"14\" height=\"14\">"; else echo "<img src=\"images/n.gif\" width=\"14\" height=\"14\">";?></TD>
							<TD align="center">
								<a href = "addedit_menu.php?menuId=<?=$row->menu_id?>&masterId=<?=$row->menumaster_id?>&langId=<?=$_REQUEST['langId']?>&pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&type=<?=$_REQUEST['type']?>&action=edit&heading=<?=$_REQUEST['heading']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>" class="smallLink">Edit</a>&nbsp;
								
								|
								<a href = "list_menus.php?menuId=<?=$row->menu_id?>&masterId=<?=$row->menumaster_id?>&langId=<?=$_REQUEST['langId']?>&pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>&action=delete" class="smallLink" onClick="return confirm('Are you sure that you want to delete the selected Record? If yes click Ok, if not click Cancel.')">Delete</a></TD> 
				...
				<table cellspacing=0 cellpadding=0 width=553 border=0 class="topColor">
					...
					<tr>
						...
						<a href="list_menus.php?pageNo=1&type=<?=$_REQUEST['type']?>&maxrows=<?=$_REQUEST['maxrows']?>&langId=<?=$_REQUEST['langId']?>&field=<?=$_REQUEST['field']?>&masterId=<?=$_REQUEST['masterId']?>&heading=<?=$_REQUEST['heading']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>
						<a href="list_menus.php?pageNo=<?=$prev?>&type=<?=$_REQUEST['type']?>&maxrows=<?=$_REQUEST['maxrows']?>&langId=<?=$_REQUEST['langId']?>&field=<?=$_REQUEST['field']?>&masterId=<?=$_REQUEST['masterId']?>&heading=<?=$_REQUEST['heading']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
						...
						<select name="pageNo" class="paragraph"  onChange="form.submit()">
							...
							?>
							 ...
							 of <?=$noOfPage?>]
							 <a href="list_menus.php?pageNo=<?=$next?>&type=<?=$_REQUEST['type']?>&field=<?=$_REQUEST['field']?>&maxrows=<?=$_REQUEST['maxrows']?>&langId=<?=$_REQUEST['langId']?>&masterId=<?=$_REQUEST['masterId']?>&heading=<?=$_REQUEST['heading']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>
							<a href="list_menus.php?pageNo=<?=$noOfPage?>&type=<?=$_REQUEST['type']?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&langId=<?=$_REQUEST['langId']?>&masterId=<?=$_REQUEST['masterId']?>&heading=<?=$_REQUEST['heading']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
