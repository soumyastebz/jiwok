...
*****************************************************************************/
	...
	*/
	...
	$sqlStart = "SELECT payment.payment_userid AS user_id,user_master.user_fname,user_master.user_lname , count(payment.payment_userid) AS count, payment.payment_expdate AS expDate, SUM(payment.payment_amount) AS totPay ";
	$fromSql  = " FROM user_master,payment ";
	$whereSql = " WHERE user_master.user_id = payment.payment_userid ";
	//sql for tot payment
	$sqlPay		=	" SELECT SUM(payment.payment_amount) AS totPayment ";
	//Total num of jiwok users
	$sqlTotNum = "SELECT DISTINCT(user_master.user_id) AS cnt from user_master,payment WHERE user_master.user_id = payment.payment_userid  AND payment.payment_status = 1 ".$joinquery;
	...
	*/
	...
	$sql = $sqlStart.$fromSql.$whereSql. $joinquery. " GROUP BY payment.payment_userid ORDER BY user_master.user_fname ASC";
...
$sqlPay.=$fromSql.$whereSql .$joinquery;
				  ...
				  <TABLE class="listTableStyle" cellSpacing=1 cellPadding=2 width="553">
                    <TBODY>
                      <TR class="tableHeaderColor">
                        ...
                        <TD width="22%" align="center" >No of payment  </TD>
						  <TD width="22%" align="center" >Total payment  </TD>
                        <TD width="27%" align="center" >Payment expiry date </TD>
                      </TR>
                      <?php if($errMsg != ""){?>
                      ...
                      <tr class="listingTable">
                        <TD align="center" colspan="5" class="tblbackgnd"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                            ...
                            <tr  <? if(($key%2) ==1){?> bgcolor="#FFFFFF" <? } ?> >
                              ...
                              <TD width="21%" height="19" align="center" style="padding-left:10px;"><? echo $val['count']?></TD>
							   <TD width="23%" height="19" align="center" style="padding-left:10px;"><? echo $val['totPay']?></TD>
                              <TD width="27%" height="19" align="left" style="padding-left:10px;"><? echo $val['expDate']?></TD>
                            </tr>
					...
					<tr><?php if($noOfPage > 1) { ?>
						...
						<a href="report_payment.php?pageNo=1&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>">
						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>
						<a href="report_payment.php?pageNo=<?=$prev?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>">
						...
						<select name="pageNo" class="paragraph"  onChange="form.submit()">
							...
							?>
							 ...
							 of <?=$noOfPage?>]
							 <a href="report_payment.php?pageNo=<?=$next?>&langId=<?=$lanId?>&field=<?=$_REQUEST['field']?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>">
							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>
							<a href="report_payment.php?pageNo=<?=$noOfPage?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>">
