...
<script language="JavaScript" type="text/javascript">
	...
	*/
	...
	if($_POST['update']=='Generate')
	{
	  ...
	  $period=$month." months";$amt=$month*$fee;
	   /*switch($month)
	   {
	   ...
	   {  
	   	  	//echo "4";
			$result1=$obj->_insertgiftadmin($period,$amt,$gcode[$i]);
			if($result1== false)
			{
			  $message="";
			}
			   ...
			   <form name="frmfaqs" action="manage_giftcode.php" method="post">
                     ...
                     <table  cellspacing=0 cellpadding=0 width="553" border=0 class="topColor">
					  ...
					  <TR>
					   ...
					   Period:
					     <select class="paragraph2" onChange="this.form.submit()" name="keyab">
					   <option <?php if($keyab=='0') {?> selected="selected"<?php }?>value="0">All Type</option>
					   <?php for($i=1;$i<=12;$i++)
							   {?><option value="<?php echo $i;?>" <?php if($keyab==$i) {?> selected="selected"<?php }?>><?=$i." months"?></option>
							 <?php }?>
					   </select>
					   </td>
				...
				<table cellspacing=0 cellpadding=0 width=553 border=0 class="topColor">
					...
					<tr>
						...
						<a href="manage_giftcode.php?pageNo=1&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keywordab=<?=$objGen->_output($_REQUEST['keywordab'])?>&keyab=<?=$objGen->_output($_REQUEST['keyab'])?>">
						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>
						<a href="manage_giftcode.php?pageNo=<?=$prev?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keywordab=<?=$objGen->_output($_REQUEST['keywordab'])?>&keyab=<?=$objGen->_output($_REQUEST['keyab'])?>">
						...
						<select name="pageNo" class="paragraph"  onChange="form.submit()">
							...
							?>
							 ...
							 of <?=$noOfPage?>]
							 <a href="manage_giftcode.php?pageNo=<?=$next?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&field=<?=$_REQUEST['field']?>&maxrows=<?=$_REQUEST['maxrows']?>&keywordab=<?=$objGen->_output($_REQUEST['keywordab'])?>&keyab=<?=$objGen->_output($_REQUEST['keyab'])?>">
							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>
							<a href="manage_giftcode.php?pageNo=<?=$noOfPage?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keywordab=<?=$objGen->_output($_REQUEST['keywordab'])?>&keyab=<?=$objGen->_output($_REQUEST['keyab'])?>">
...
<tr> 
              ...
              <tr> 
			   ...
			   <form name="frmgcode" action="manage_giftcode.php" method="post">
                     ...
                     <table  cellspacing=0 cellpadding=0 width="577" border=0 class="topColor">
                      <tbody>
					  <?php if($errMsg2nd != ""){?>
						  <TR class="listingTable"> 
					 ...
					 <?php }?>
					  <TR><th width="161" height="43"  align="left"> Code List</th>
					   ...
					   <?php for($i=1;$i<=12;$i++)
							   {?><option value="<?php echo $i;?>" <?php if($keyb==$i) {?> selected="selected"<?php }?>><?=$i." months"?></option>
							 <?php }?>
					   </select>
					  &nbsp;&nbsp;&nbsp;
  ...
  Calendar.setup(
    {
      ...
      button      : "to_date_btn"       // ID of the button
				  ...
				  <TABLE class="listTableStyle" cellSpacing=1 cellPadding=2 width="565">
				   <TBODY> 
					   <TR class="tableHeaderColor">
						<TD width="11%" align="center" ><input type="button" onClick="javascript:CheckAll(document.frmgcode.unused)" value="Select All">&nbsp;/&nbsp;
						  <input name="button" type="button" onClick="javascript:UnCheckAll(document.frmgcode.unused)" value="Un Select All"></TD>
						<TD width="19%" >Gift Code</TD>
						<TD width="13%" >Code Type</TD>
						<TD width="25%" >Bought</TD>
						<TD width="22%" align="center" >Used</TD>
				...
				<table cellspacing=0 cellpadding=0 width=569 border=0 class="topColor">
					...
					<tr>
						...
						<a href="manage_giftcode.php?pageNob=1&typeb=<?=$_REQUEST['typeb']?>&langIdb=<?=$lanIdb?>&maxrowsb=<?=$_REQUEST['maxrowsb']?>&fieldb=<?=$_REQUEST['fieldb']?>&keywordb=<?=$_REQUEST['keywordb']?>&keyb=<?=$_REQUEST['keyb']?>&searchkey=<?=$_REQUEST['searchkey']?>&from_date=<?=$_REQUEST['from_date']?>&to_date=<?=$_REQUEST['to_date']?>">
						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>
						<a href="manage_giftcode.php?pageNob=<?=$prevb?>&typeb=<?=$_REQUEST['typeb']?>&langIdb=<?=$lanIdb?>&maxrowsb=<?=$_REQUEST['maxrowsb']?>&fieldb=<?=$_REQUEST['fieldb']?>&keywordb=<?=$_REQUEST['keywordb']?>&keyb=<?=$_REQUEST['keyb']?>&searchkey=<?=$_REQUEST['searchkey']?>&from_date=<?=$_REQUEST['from_date']?>&to_date=<?=$_REQUEST['to_date']?>">
						<img src="images/mail-arrowlg.gif" border=0 width="20" height="20" align="absmiddle" alt="Previous Page"></a>[Page 
						<select name="pageNob" class="paragraph"  onChange="form.submit()">
							...
							?>
							 ...
							 of <?=$noOfPageb?>]
							 <a href="manage_giftcode.php?pageNob=<?=$nextb?>&typeb=<?=$_REQUEST['typeb']?>&langIdb=<?=$lanIdb?>&fieldb=<?=$_REQUEST['fieldb']?>&maxrowsb=<?=$_REQUEST['maxrowsb']?>&keywordb=<?=$_REQUEST['keywordb']?>&keyb=<?=$_REQUEST['keyb']?>&searchkey=<?=$_REQUEST['searchkey']?>&from_date=<?=$_REQUEST['from_date']?>&to_date=<?=$_REQUEST['to_date']?>">
							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>
							<a href="manage_giftcode.php?pageNob=<?=$noOfPageb?>&typeb=<?=$_REQUEST['typeb']?>&langIdb=<?=$lanIdb?>&maxrowsb=<?=$_REQUEST['maxrowsb']?>&fieldb=<?=$_REQUEST['fieldb']?>&keywordb=<?=$_REQUEST['keywordb']?>&keyb=<?=$_REQUEST['keyb']?>&searchkey=<?=$_REQUEST['searchkey']?>&from_date=<?=$_REQUEST['from_date']?>&to_date=<?=$_REQUEST['to_date']?>">
							<img src="images/mail-arrowlastg.gif" border=0 width="20" height="20" align="absmiddle" alt="Last Page"></a>
						</td>
