<?php
	...
	if($_REQUEST['langId']!="")
	  ...
	  $lanId=1;  
	$vocalId=$siteMasterMenuConfig['VOCALS_ID'];
	
	$vocalObj = 	new  Vocals($lanId,$vocalId);
	$genObj   =	new General();
	...
	if($_REQUEST['field']){
		$field = 'vocal_manager.'.$_REQUEST['field'];
		$type = $_REQUEST['type'];
	}else{
		$field = "vocal_manager. vocal_name";
		$type = "ASC";
	}
	...
	if(trim($_REQUEST['keyword'])){
		...
		$searchQuery	=	" vocal_manager.vocal_name like '%".$cleanData."%'";	}	
	}
	...
	if($_REQUEST['action'] == "delete"){
		   ...
		   if($_REQUEST['deleteId']){
			    ...
			    $vocalObj->_deleteVocal($_REQUEST['deleteId']);
				$confMsg = "Successfully Deleted";
			}
		
	}	
	
	$query = "SELECT count(*) as max FROM vocal_manager WHERE  vocal_manager.language_id=".$lanId."";
	...
	if($_REQUEST['pageNo']){
		...
		$result=$vocalObj->_showPage($totalRecs,$_REQUEST['pageNo'],$_REQUEST['maxrows'],$field,$type,$searchQuery);
	}
	else{
...
<tr> 
              ...
              <tr> 
			   ...
			   <form name="frmadmin" action="list_vocals.php" method="post">
				  ...
				  <tr>
					...
					<TD align="left">
						...
						<? if($genObj->_output($_REQUEST['keyword'])){ ?>
							<td valign="middle" width="50"><a href="list_vocals.php?maxrows=<?=$_REQUEST['maxrows']?>"><img src="images/list.gif" alt="Listing Record">&nbsp;List </a></td>
						<? }else{ ?>
							<td valign="middle" width="50" class="noneAnchor"><img src="images/list.gif" alt="Listing Record">&nbsp;List </td>
						<? } ?>
						<td valign="middle"><a href="addedit_vocals.php?action=add&pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&type=<?=$_REQUEST['type']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>"><img src="images/add.gif" border="0" alt="Add Record">&nbsp;Add   </a></td>
                     ...
                     <table  cellspacing=0 cellpadding=0 width="553" border=0 class="topColor">
                      <tbody>
					   ...
					   <td width="58" valign=top class="paragraph2">Language</td>
					   <td width="98" valign=top class="paragraph2"><select name="langId" class="paragraph" onChange="this.form.submit()">
                                <?
									$string = "";
				     ...
				     <table class="listTableStyle" cellspacing=1 cellpadding=2 width="553">
                       <tbody>
                         <tr class="tableHeaderColor">
                           ...
                           <td width="61%" >Vocal Name <a href="list_vocals.php?field=vocal_name&type=asc&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>"><img src="images/up.gif" border="0" alt="Ascending Sort">&nbsp;</a> <a href="list_vocals.php?field=vocal_name&type=desc&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>"><img src="images/down.gif" border="0" alt="Descending Sort"></a> </td>
						...
						?>
                         <tr class="listingTable">
                           ...
                           <td><?=$genObj->_output($row->vocal_name);?></td>
                           
                           <td align="center"><a href = "addedit_vocals.php?elementId=<?=$row->vocal_id?>&pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&type=<?=$_REQUEST['type']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>&action=edit" class="smallLink">Edit</a>&nbsp;
                               
                             | <a href = "list_vocals.php?deleteId=<?=$row->vocal_id?>&pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&langId=<?=$lanId?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>&action=delete" class="smallLink" onClick="return confirm('Are you sure that you want to delete the selected Record? If yes click Ok, if not click Cancel.')">Delete</a></td>
					...
					<tr>
						...
						<a href="list_vocals.php?pageNo=1&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>
						<a href="list_vocals.php?pageNo=<?=$prev?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
						...
						<select name="pageNo" class="paragraph"  onChange="form.submit()">
							...
							?>
							 ...
							 of <?=$noOfPage?>]
							 <a href="list_vocals.php?pageNo=<?=$next?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&field=<?=$_REQUEST['field']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>
							<a href="list_vocals.php?pageNo=<?=$noOfPage?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
