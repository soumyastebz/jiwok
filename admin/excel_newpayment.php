 ...
 //for generating the excell sheet for the report
	...
	if($tab	==	1	&&	count($memberList)>0)
	{
		...
		$amountQuery	=	"SELECT sum( PS.payment_amount ) AS Total, PS.payment_currency FROM payment AS PS LEFT 					 							JOIN user_master AS UM ON UM.user_id = PS.payment_userid left join brand_user b on   									  							b.user_id=UM.user_id left join brand_master c on c.brand_master_id=b.brand_master_id
							WHERE PS.payment_status = '1' AND PS.version = 'New' $whereSql GROUP BY 																			 							PS.payment_currency";
		...
		$PlanQuery		=	"SELECT sum( PS.payment_amount ) AS Total,COUNT( PS.payment_amount ) As count,      	 							PS.payment_currency,PS.payment_amount FROM payment AS PS LEFT JOIN user_master AS UM ON 							UM.user_id = PS.payment_userid left join brand_user b on b.user_id=UM.user_id 
							left join brand_master c on c.brand_master_id=b.brand_master_id WHERE PS.payment_status 							= '1' AND PS.version = 'New' $whereSql GROUP BY PS.payment_amount ORDER BY                				`PS`.`payment_currency` DESC";
	...
	if(($tab	==	2	||	$tab	==	3)	&&	count($memberList)>0)
	{
		...
		if($tab	==	2)
		{
			$amountQuery	=	"SELECT sum( PS.payment_amount ) AS Total, PS.payment_currency FROM                           		payment_transactions AS PC INNER JOIN payment AS PS ON PC.payment_id= PS.payment_id
								INNER JOIN user_master AS UM ON UM.user_id = PC.user_id left join brand_user b on           						b.user_id=UM.user_id left join brand_master c on c.brand_master_id = 								 								b.brand_master_id WHERE PS.payment_status='0' AND PS.version='New' AND              					PC.status='CANCELLED' $whereSql GROUP BY PS.payment_currency";					
			$resultAmount	=	$objDb->_getList($amountQuery);	
			$PlanQuery		=	"SELECT sum( PS.payment_amount ) AS Total,COUNT( PS.payment_amount ) As	count, 																										 								PS.payment_currency,PS.payment_amount FROM payment_transactions AS PC INNER JOIN  							 								payment AS PS ON PC.payment_id = PS.payment_id INNER JOIN user_master AS UM ON             					UM.user_id = PC.user_id left join brand_user b on b.user_id=UM.user_id 
								...
								WHERE PS.payment_status='0' AND PS.version='New' AND PC.status='CANCELLED'  		 								$whereSql GROUP BY PS.payment_amount ORDER BY `PS`.`payment_currency` DESC";	
			$resultPlan		=	$objDb->_getList($PlanQuery);
