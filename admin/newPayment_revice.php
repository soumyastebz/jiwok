<?php
	...
	if($_REQUEST['action'] == "cancel"){
		   ...
		   if($res == '00000')
		   {
				...
				$resUserQry	=	$payObj->dbSelectOne($userQry);
				if($resUserQry['user_language']	!=	5)
				{				
					$sqlQryPr  	=	"SELECT * FROM `payment_cronjob` where user_id='".$resQry['user_id']."' AND status='VALID'";
					$resQryPr	=	$payObj->dbSelectOne($sqlQryPr);
					
					/*$dbValues	=	"payment_expiry_date='".date('Y-m-d')."'";
					$dbCond		=	"id='".$resQryPr['id']."'";
					$dbTable	=	"payment_cronjob";
					$payObj->dbUpdate($dbTable,$dbValues,$dbCond);*/
					$res		=	$payObj->unsubsriptionFromPaybox($resQryPr['pp_id']);
...
//**************************
	$query = "SELECT count(*) as max,PC.*,PD.payment_currency,PD.payment_amount,UM.user_email FROM payment_transactions PC LEFT JOIN payment PD ON PC.payment_id = PD.payment_id LEFT JOIN user_master UM on UM.user_id = PD.payment_userid WHERE PC.status = 'PAID' ".$joinquery. " AND PD.payment_status = 1";
	...
	if($_REQUEST['pageNo']){
		...
		$result = $payObj->_showPage($totalRecs,$_REQUEST['pageNo'],$_REQUEST['maxrows'],$field,$type,$searchQuery,$page_name);
	}
	else{
...
<tr> 
              ...
              <tr> 
			   ...
			   <form name="frmadmin" action="newPayment_revice.php" method="post">
				  ...
				  <tr>
					...
					<tr>
						...
						<a href="newPayment_revice.php?pageNo=1&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>
						<a href="newPayment_revice.php?pageNo=<?=$prev?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
						...
						<select name="pageNo" class="paragraph"  onChange="form.submit()">
							...
							?>
							 ...
							 of <?=$noOfPage?>]
							 <a href="newPayment_revice.php?pageNo=<?=$next?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&field=<?=$_REQUEST['field']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>
							<a href="newPayment_revice.php?pageNo=<?=$noOfPage?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
