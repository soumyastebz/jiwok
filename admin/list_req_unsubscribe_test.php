<?php
   ob_start();
	...
	include_once('../includes/classes/class.member_test.php');
	include_once('../includes/classes/class.DbAction.php');
	
	include_once('../includes/classes/class.Languages.php');
	include_once('../includes/classes/class.Parser.php');
	...
	$parObj 		=   new Contents('list_req_unsubscribe.php');
	...
	$heading = "Unsubscribe membership";
	$errMesg = "";
	$confMsg = "";
	...
	//Unsubscibe membership
	
	if($_REQUEST['action'] == "unsubscribe"){
	       $xmlPath="../xml/french/page.xml";
		   if($_REQUEST['userId']){
		        $userdetails=$memObj->_getOneUser($_REQUEST['userId']);
				if($userdetails)
				...
				$unscubscribeMail[$_REQUEST['userId']] = "\n".$parObj->_getLabenames($arrayData,'reg_mail_to','name').",\n\n";
				$unscubscribeMail[$_REQUEST['userId']] .= $parObj->_getLabenames($arrayData,'unscubscribe_mail_part1','name')."\n";
				$unscubscribeMail[$_REQUEST['userId']] .= $parObj->_getLabenames($arrayData,'unscubscribe_mail_part2','name')."\n";
				$unscubscribeMail[$_REQUEST['userId']] .= $parObj->_getLabenames($arrayData,'unscubscribe_mail_part3','name')." \n\n";
				
				$unscubscribeMail[$_REQUEST['userId']] .= $parObj->_getLabenames($arrayData,'unscubscribe_mail_part4','name')." \n";
				$unscubscribeMail[$_REQUEST['userId']] .= $parObj->_getLabenames($arrayData,'unscubscribe_mail_part5','name').": ";
				$unscubscribeMail[$_REQUEST['userId']] .= $parObj->_getLabenames($arrayData,'unscubscribe_mail_partUrl6','name')."\n\n";
				
				$unscubscribeMail[$_REQUEST['userId']] .= $parObj->_getLabenames($arrayData,'unscubscribe_mail_footer1','name')."\n\n";
				$unscubscribeMail[$_REQUEST['userId']] .= $parObj->_getLabenames($arrayData,'unscubscribe_mail_footer2','name').": ";
				$unscubscribeMail[$_REQUEST['userId']] .= $parObj->_getLabenames($arrayData,'unscubscribe_mail_footer3Url','name')."\n\n ";
				$unscubscribeMail[$_REQUEST['userId']] .= $parObj->_getLabenames($arrayData,'unscubscribe_mail_footer4','name')."\n\n";
				$unscubscribeMail[$_REQUEST['userId']] .= $parObj->_getLabenames($arrayData,'unscubscribe_mail_footer5','name')."\n\n";
				$subject_mail[$_REQUEST['userId']]=$parObj->_getLabenames($arrayData,'subject','name')."\n\n";
				
				$unSub_member = array();
				$unSub_member[] = $_REQUEST['userId'];
				$unSub_member	=	implode(',',$unSub_member);
					$returnValue=$memObj->_unsubscribeUserMemebership($unSub_member);
					//$returnValue=1;
					if($returnValue == 1 )
					{
					$mails = $memObj->_sendUnsubscribeMails($unSub_member, $unscubscribeMail,$subject_mail);
					$confMsg = "Successfully unsubscribed membership";
					}
					else
					$errMsg="Please select atleast one record for unsubscribe!";
			}
		
	}	
	//Unsubscibe membership
	$returnValue='';
	if($_REQUEST['unsubscribeRec']){
	$userIds	=	implode(',',$_REQUEST['chkdelete']);
	$returnValue=$memObj->_unsubscribeUserMemebership($userIds); 
	 //$returnValue=1;
		if($returnValue == 0){ 
		$errMsg="Please select atleast one record for unsubscribe!";
		} 
		else { foreach($_REQUEST['chkdelete'] as $userId)
		        {
		         //echo "hi".$userId;
		 ...
		 $mails = $memObj->_sendUnsubscribeMails($userIds, $unscubscribeMail,$subject_mail); 
		$confMsg = "Successfully unsubscribed membership";
		}
	}
	
	$query = "SELECT count(*) as max FROM user_master WHERE user_unsubscribed = 1 ";
	if($searchQuery)
		$query .= $searchQuery; 
	...
	if($_REQUEST['pageNo']){
		...
		$result = $memObj->_showReqUnsubPage($_REQUEST['pageNo'],$_REQUEST['maxrows'],$field,$type,$searchQuery);
	}
	else{
...
<tr> 
              ...
              <tr> 
			   ...
			   <form name="frmadmin" action=" list_req_unsubscribe.php" method="post">
				  ...
				  <tr>
					...
					<TD align="left">
						...
						<? if($genObj->_output($_REQUEST['keyword'])){ ?>
							<td valign="middle" width="50"><a href=" list_req_unsubscribe.php?maxrows=<?=$_REQUEST['maxrows']?>"><img src="images/list.gif" alt="Listing Record">&nbsp;List </a></td>
						<? }else{ ?>
							<td valign="middle" width="50" class="noneAnchor"><img src="images/list.gif" alt="Listing Record">&nbsp;List </td>
						<? } ?>
						<td valign="middle"><a href="list_unsubscribedMembers.php"><img src="images/add.gif" border="0" alt="Add Record">&nbsp;Unsubscribed Members   </a></td>
				     ...
				     <table class="listTableStyle" cellspacing=1 cellpadding=2 width="553">
                       <tbody>
                         <tr class="tableHeaderColor">
                           ...
                           <td width="28%" align="left">&nbsp; Member Name <a href=" list_req_unsubscribe.php?field=user_fname&type=asc&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>"><img src="images/up.gif" border="0" alt="Ascending Sort">&nbsp;</a> <a href=" list_req_unsubscribe.php?field=user_fname&type=desc&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>"><img src="images/down.gif" border="0" alt="Descending Sort"></a> </td>
                           <td width="27%" align="center"> Date Of Request 
						   <a href=" list_req_unsubscribe.php?field=user_req_unsubscribe&type=asc&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>"><img src="images/up.gif" border="0" alt="Ascending Sort">&nbsp;</a> <a href=" list_req_unsubscribe.php?field=user_req_unsubscribe&type=desc&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>"><img src="images/down.gif" border="0" alt="Descending Sort"></a>						   </td>
						...
						?>
                         <tr class="listingTable">
                           ...
                           <td  align="center"><?=$genObj->_modifier_date_format($row->user_req_unsubscribe);?></td>
                           <td align="center"> <a href = "view_req_member.php?userId=<?=$row->user_id?>&pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&type=<?=$_REQUEST['type']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>&action=view&pageName=1" class="smallLink">View</a>&nbsp;       
                           |&nbsp;<a href = "list_req_unsubscribe.php?userId=<?=$row->user_id?>&pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>&action=unsubscribe" class="smallLink" onClick="return confirm('Are you sure that you want to unsubscribe this member? If yes click Ok, if not click Cancel.')">Unsubscribe</a>					 </td>
                         </tr>
                         <?php
						$count++;
						}
					...
					<tr>
						<td align="left" colspan = "3" class="leftmenu" style="padding-left:5px; padding-top:5px;"><span class="leftmenu" style="padding-left:5px; padding-top:5px;">
						  <input type="submit" name="unsubscribeRec" value="Unsubscribe Members" onClick="return confirm('Are you sure that you want to unsubsribe member(s)? If Yes click OK, or click Cancel.')" style="cursor:pointer;">
						</span></td>
						<?php if($noOfPage > 1) { ?>
						<td width="297" colspan = "3" align="right" class="leftmenu" style="padding-left:5px; padding-top:5px;">
						<a href=" list_req_unsubscribe.php?pageNo=1&type=<?=$_REQUEST['type']?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>
						<a href=" list_req_unsubscribe.php?pageNo=<?=$prev?>&type=<?=$_REQUEST['type']?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
						...
						<select name="pageNo" class="paragraph"  onChange="form.submit()">
							...
							?>
							 ...
							 of <?=$noOfPage?>]
							 <a href=" list_req_unsubscribe.php?pageNo=<?=$next?>&type=<?=$_REQUEST['type']?>&field=<?=$_REQUEST['field']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>
							<a href=" list_req_unsubscribe.php?pageNo=<?=$noOfPage?>&type=<?=$_REQUEST['type']?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
