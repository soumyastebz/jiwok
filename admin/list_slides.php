...
<tr> 
              ...
              <tr> 
			   ...
			   <form name="frmservices" action="list_slides.php" method="post">
				  ...
				  <tr>
					...
					<TD align="left">
						...
						<? } ?>
						<td valign="middle" width="50"><!--<a href="addedit_slides.php?action=add&pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>"><img src="images/add.gif" border="0" alt="Add Record">&nbsp;Add   </a> --></td>
					...
					<!--	<TD width="8%" align="center" >Status</TD> -->
					 ...
					 <?php }
						...
						?>
							...
							<TD><?=stripslashes(stripslashes(stripslashes($row['slide_content'])));?></TD>
							...
							<TD align="center">
								<a href = "addedit_slides.php?slideId=<?=$row['slide_id']?>&pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&type=<?=$_REQUEST['type']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>&action=edit" class="smallLink">Edit</a>&nbsp;
						<?php if(count($result) > 4)  { ?>		|
								<a href = "list_slides.php?slideId=<?=$row['slide_id']?>&pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>&action=delete" class="smallLink" onClick="return confirm('Are you sure that you want to delete the selected Record? If yes click Ok, if not click Cancel.')">Delete</a></TD> 
				<?php } ?>				
						    </tr>
						<?php
						$count++;
