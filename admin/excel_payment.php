<? 
 //for generating the excell sheet for the report
	...
	$sql = "SELECT payment.payment_userid AS user_id, user_fname, user_gender, user_lname, user_email, user_address, user_city, user_state, user_country, DATE_FORMAT(user_doj,'%W %D %M %Y') AS date, count(payment.payment_userid) AS count, payment.payment_expdate AS expDate FROM ".$PostQuerry;
	...
	$worksheet->write(0, 1, "Payment Report ", $format_title);
	...
	$worksheet->write(1,8, 'Number Of Payment', $format_bold);
	$worksheet->write(1,9, 'Payment Expiry Date', $format_bold);
	$worksheet->write(1,10, 'Last Login', $format_bold);
	...
	if(count($memberList)>0){
		...
		foreach($memberList as $key=>$val){
			if(count($val['user_fname']) > 0){
				...
				if(sizeof($coutryList)>0){
					$worksheet->write($row,7, $coutryList[0]['countries_name']);
				}
				$worksheet->write($row,8, $val['count']);
				$worksheet->write($row,9, $val['expDate']);
				...
				$worksheet->write($row,10, $inacList[0]['date_inac']);
				$index++;
			}
		}
	}
