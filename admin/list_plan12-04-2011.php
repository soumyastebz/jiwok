...
*****************************************************************************/
	...
	include_once('../includes/classes/class.plan.php'); 
	...
	*/ 
	...
	if($_REQUEST['field']){
		$field = "plan_manager.".$_REQUEST['field'];
		$type = $_REQUEST['type'];
	}else{
		$field = "plan_id";
		$type = "ASC";
	...
	if($_REQUEST['action'] == "delete"){
		$id		 = $_REQUEST['planId'];
		$result	 = $objPlan->_deletePlan($id);
		$confMsg = "Successfully Deleted";
	}	
	
	$totalRecs = $objPlan->_getTotalCount($searchQuery,$lanId);
	if($totalRecs <= 0)
		$errMsg = "No Records";
	...
	if($_REQUEST['pageNo']){
		...
		$result =  $objPlan->_showPage($totalRecs,$_REQUEST['pageNo'],$_REQUEST['maxrows'],$field,$type,$searchQuery);
	}
	else{
	...
	if($totalRecs <= $_REQUEST['pageNo']*$_REQUEST['maxrows'])
	{
		...
		$endNo = $totalRecs;
		$displayString = "Viewing $startNo to $endNo of $endNo Plans";
	...
	else
	{
		...
		$displayString = "Viewing $startNo to $endNo of $totalRecs Plans";
...
<BODY class="bodyStyle"> echo higgeggsggsgsi; exit;
			   ...
			   <form name="frmplan" action="list_plan.php" method="post">
				  ...
				  <TABLE class="listTableStyle" cellSpacing=1 cellPadding=2 width="553">
				   <TBODY> 
					   <TR class="tableHeaderColor">
						...
						<TD width="32%" ><?=$heading;?>&nbsp;Name
						
						<a href="list_plan.php?field=manager_question&type=asc&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>"><img src="images/up.gif" border="0" alt="Ascending Sort">&nbsp;</a>
						<a href="list_plan.php?field=manager_question&type=desc&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>"><img src="images/down.gif" border="0" alt="Descending Sort"></a>
					 ...
					 <?php }
						...
						?>
							...
							<TD><?=stripslashes(stripslashes(stripslashes($row['plan_name'])));?></TD>
							<TD><?=stripslashes(stripslashes(stripslashes($row['plan_amount'])));?></TD>
							
							<TD align="center"><?php if($row['plan_status'] == 1) echo "<img src=\"images/y.gif\" width=\"14\" height=\"14\">"; else echo "<img src=\"images/n.gif\" width=\"14\" height=\"14\">";?></TD>
							<TD align="center">
								<a href = "addedit_plan.php?planId=<?=$row['plan_id']?>&pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&type=<?=$_REQUEST['type']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>&action=edit" class="smallLink">Edit</a>&nbsp;
								|
								<a href = "list_plan.php?planId=<?=$row['plan_id']?>&pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>&action=delete" class="smallLink" onClick="return confirm('Are you sure that you want to delete the selected Record? If yes click Ok, if not click Cancel.')">Delete</a></TD> 
				...
				<table cellspacing=0 cellpadding=0 width=553 border=0 class="topColor">
					...
					<tr>
						...
						<a href="list_plan.php?pageNo=1&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>">
						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>
						<a href="list_plan.php?pageNo=<?=$prev?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>">
						...
						<select name="pageNo" class="paragraph"  onChange="form.submit()">
							...
							?>
							 ...
							 of <?=$noOfPage?>]
							 <a href="list_plan.php?pageNo=<?=$next?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&field=<?=$_REQUEST['field']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>">
							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>
							<a href="list_plan.php?pageNo=<?=$noOfPage?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>">
