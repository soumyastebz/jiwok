...
*****************************************************************************/
	...
	*/
	...
	if($_POST['add']||$_POST['update']){
		...
		if($_POST['add'])	{
			...
			while(list($key,$value) = each($languageArray)){
				...
				$check	= $objService->_isserviceExists_new($objGen->_clean_data($_REQUEST['service_content'.$key]));
				if($check) 
		...
		if($_POST['update'])	{
			...
			while(list($key,$value) = each($languageArray)){ 
				...
				$check = false;
				$check	= $objService->_isserviceExists_new($objGen->_clean_data($_REQUEST['service_content'.$key]),$_REQUEST['serviceId']);
				if($check) 
	...
	if($_POST['add']){
		...
		if(count($errorMsg)==0)	{
			$nextId = $objService->_insertMaster_new($_POST['service_status']);
			reset($languageArray);
			while(list($key,$value) = each($languageArray)){
				$title	= $_POST['service_content_'.$key];
				$body	= $_POST['service_description_'.$key];
				$status	= $_POST['service_status'];
				$elmts	= array("service_content" => $title,"service_description" => $body,"service_status" => $status,"language_id" => $key);
				
				$result =  $objService->_insertService_new($elmts,count($languageArray));
			}
			header("Location:list_services_new.php?status=success_add&langId=".$_REQUEST['langId']."&pageNo=".$_REQUEST['pageNo']."&maxrows=".$_REQUEST['maxrows']."&type=".$_REQUEST['type']."&field=".$_REQUEST['field']."&keyword=".$_REQUEST['keyword']);
		}
	}
	...
	if($_POST['update']){
		...
		if(count($errorMsg)==0)	{
			reset($languageArray);
			while(list($key,$value) = each($languageArray)){
					$query = "SELECT  count(*) as dataCount FROM service_manager WHERE servicemaster_id =".$_REQUEST['serviceId']." AND language_id=".$key."";
					...
					if(DB::isError($result)) 
						...
						if($coundRecs >= 1){
							...
							$body	= $_POST['service_description_'.$key];
							$status	= $_POST['service_status'];
							$elmts	= array("service_content" => $title,"service_description" => $body,"service_status" => $status);
							$result = $objService->_updateService_new($_REQUEST['serviceId'],$key,$elmts);
						}
						else
						{
						    $title	= $_POST['service_content_'.$key];
							$body	= $_POST['service_description_'.$key];
							$result = $objService->_insertOneService_new($_REQUEST['serviceId'],$key,$title,$body);
						}
				
			}
			//print_r($title);
			
			header("Location:list_services_new.php?status=success_update&langId=".$_REQUEST['langId']."&pageNo=".$_REQUEST['pageNo']."&maxrows=".$_REQUEST['maxrows']."&type=".$_REQUEST['type']."&field=".$_REQUEST['field']."&keyword=".$_REQUEST['keyword']);
			
		}
	}
}	
	//if edit following will execute on loading
	if($_REQUEST['serviceId'] and count($errorMsg)==0){
		//Some security check here
		$result = $objService->_getAllById_new($_REQUEST['serviceId']);
...
<tr> 
              ...
              <tr> 
				...
				<form name="serviceform" action="addedit_services_new.php" method="post" onSubmit="return formChecking()">
                            ...
                            <TD valign="top">
								  ...
								  <tr>
									...
									<TD align="left">
										...
										<table height="50" class="topActions"><tr><td valign="middle" width="50"><a href="list_services_new.php?pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$_REQUEST['keyword']?>"><img src="images/list.gif" border="0" alt="Listing Record">&nbsp;List   </a></td>
				     ...
				     <? 
						...
						while(list($key,$val) = each($languageArray)){
							if(count($result) != 0){
								...
								$_POST['service_content_'.$key] = "";
								$_POST['service_description_'.$key]  =  "";
								$_POST['service_status']  =  "";
								foreach($result	as $results)
								{
									if($results[language_id]	==	$key)
									{
										$_POST['service_content_'.$key] = stripslashes(stripslashes(stripslashes($results['service_content'])));
										$_POST['service_description_'.$key]  =  stripslashes(stripslashes(stripslashes($results['service_description'])));
										$_POST['service_status']  =  $results['service_status'];
									}
								}
							}
					?> 
