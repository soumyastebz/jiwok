...
$combo	=	$objService->get_combo_arr('category',$getAllTrainCats,'flex_id','category_name',$fid,$params="");
	...
	*/
	...
	if($_POST['add']){
		...
		if(count($errorMsg)==0)	{
			$nextId = $objService->_insertMaster($_POST['category'],$_POST['service_status']);
		/*reset($languageArray);
			while(list($key,$value) = each($languageArray)){
			...
			header("Location:list_category.php?status=success_add&langId=".$_REQUEST['langId']."&pageNo=".$_REQUEST['pageNo']."&maxrows=".$_REQUEST['maxrows']."&type=".$_REQUEST['type']."&field=".$_REQUEST['field']."&keyword=".$_REQUEST['keyword']);
		}
	}
...
$result = $GLOBALS['db']->getAll($query, DB_FETCHMODE_OBJECT);
					...
					if(DB::isError($result)) 
						...
						if($coundRecs >= 1){
							...
							$elmts	= array("flex_id" => $category,"language_id" => $status);
		...
		//	}
			...
			header("Location:list_category.php?status=success_update&langId=".$_REQUEST['langId']."&pageNo=".$_REQUEST['pageNo']."&maxrows=".$_REQUEST['maxrows']."&type=".$_REQUEST['type']."&field=".$_REQUEST['field']."&keyword=".$_REQUEST['keyword']);
...
<tr> 
              ...
              <tr> 
				...
				<form name="serviceform" action="addedit_category.php" method="post" onSubmit="return formChecking()">
                            ...
                            <TD valign="top">
								  ...
								  <tr>
									...
									<TD align="left">
										...
										<table height="50" class="topActions"><tr><td valign="middle" width="50"><a href="list_category.php?pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$_REQUEST['keyword']?>"><img src="images/list.gif" border="0" alt="Listing Record">&nbsp;List   </a></td>
