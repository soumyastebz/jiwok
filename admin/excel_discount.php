<? 
 ...
 //for generating the excell sheet for the report
	...
	$sql = "SELECT user_master.user_id,user_fname,user_gender,user_lname,user_alt_email AS user_email,user_address,user_city,user_state,user_country,DATE_FORMAT(user_doj,'%W %D %M %Y') AS date,discount_users.start_date AS payDate,  discount_users.discount_code AS discCode, discount_users.payment_status AS payStatus FROM ".$PostQuerry;
...
echo "<pre/>";print_r($memberList );exit;
	...
	$worksheet->write(0, 1, " Discount Report  ", $format_title);
	...
	$worksheet->write(1,8, 'Discount Code Used', $format_bold);

	$worksheet->write(1,9, 'Payment Status', $format_bold);

	$worksheet->write(1,10, 'Paid Date', $format_bold);
	...
	if(count($memberList)>0){
		...
		$worksheet->write($row,8, $val['discCode']);

		$worksheet->write($row,9, $val['payStatus']);

		$worksheet->write($row,10, $val['payDate']);
	...
	$workbook->send('Discount_report_jiwok_'.date('d_m_Y').'.xls');
