<?php
    //project    : Jiwok
	//Module     : Referral System Activation 
	//Programmer : Soumya
	//Date       : 16-04-2013
	//########################################
	...
	$heading = "Referral System Activation";
	$errMesg = "";
	$confMsg = "";
	...
	//referral status updates here
	if($_REQUEST['langname'])
	{
		$langname=$_REQUEST['langname'];
	}
	if($_REQUEST['status'] == "inactive" && $_REQUEST['langname'] == $langname)
	{ 
		mysql_query("UPDATE languages set lang_refstatus = 0 where language_name='$langname'");
	}
	elseif($_REQUEST['status'] == "active" && $_REQUEST['langname'] == $langname)
	{ 
		mysql_query("UPDATE languages set lang_refstatus = 1 where language_name='$langname'");
	}
...
<tr> 
              ...
              <tr> 
			   ...
			   <form name="frmadmin" action="referral_lang.php" method="post">
				  ...
				  <tr>
					...
					<TD align="left">
						...
						<? if($genObj->_output($_REQUEST['keyword'])){ ?>
							<td valign="middle" width="50"><a href="referral_lang.php?maxrows=<?=$_REQUEST['maxrows']?>"><img src="images/list.gif" alt="Listing Record">&nbsp;List </a></td>
						<? }else{ ?>
						...
						while($result->fetchInto($row,DB_FETCHMODE_OBJECT)){
                         ...
                         <tr class="listingTable">
                           ...
                           <td><?=$genObj->_output($row->language_name);?></td>
                             <td align="center"><?php if($row->lang_refstatus == 1) { ?><a href="referral_lang.php?status=inactive&langname=<?=$row->language_name?>" onClick=" return changePic()";><?="<img src=\"images/y.gif\" width=\"14\" height=\"14\" id=\"Img1\">";?></a> <?php }else { ?> <a href="referral_lang.php?status=active&langname=<?=$row->language_name?>" onClick=" return changePic1()";><?php echo "<img src=\"images/n.gif\" width=\"14\" height=\"14\" id=\"Img2\">"; ?> </a><?php } ?> </td>
					...
					<tr>
						...
						<a href="referral_lang.php?pageNo=1&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>
						<a href="referral_lang.php?pageNo=<?=$prev?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
						...
						<select name="pageNo" class="paragraph"  onChange="form.submit()">
							...
							?>
							 ...
							 of <?=$noOfPage?>]
							 <a href="referral_lang.php?pageNo=<?=$next?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&field=<?=$_REQUEST['field']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>
							<a href="referral_lang.php?pageNo=<?=$noOfPage?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$genObj->_output($_REQUEST['keyword'])?>">
...
function changePic()
{
	...
	if(confirm('Are you sure that you want to make inacive Referral system for the selected Record? If yes click Ok, if not click Cancel.')== true)
	
 	{	
  		document.getElementById("Img1").src="images/n.gif";
...
function changePic1()
{
	if(confirm('Are you sure that you want to make inacive Referral system for the selected Record? If yes click Ok, if not click Cancel.')== true)
{	
  document.getElementById("Img2").src="images/y.gif";
}
	else 
	{
        return false;
