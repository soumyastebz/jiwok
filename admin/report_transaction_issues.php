...
*****************************************************************************/
	...
	*/
	...
	$heading = "Transaction Issues Report";
	...
	*/
	...
	if($_POST['daterange'] == 1){
		...
		switch($_POST['dropdown1']){
			case 'today':
				$whereSql .= " AND P.payment_date  = '".$today."'";
			...
			case 'yest':
				...
				$whereSql .= " AND P.payment_date = '".$yesterday."'";
			...
			case 'last7':
				...
				$whereSql .= " AND P.payment_date BETWEEN '".$requiredDate."' AND '".$today."'";
			...
			case 'thismonth':	
				...
				$whereSql 	 .= " AND MONTH(P.payment_date) = '".$thisMonth."'  AND YEAR(P.payment_date) = '".$thisYear."'";
			...
			case 'lastmonth':	
				...
				$whereSql 	 .= " AND MONTH(P.payment_date) = '".$lasMonth."'  AND YEAR(P.payment_date) = '".$lasYear."'";
				...
				$whereSql .= " AND YEAR(P.payment_date) = '".$lasYear."'";
		...
		if(count($errorMsg) == 0){
			if($startDate != $endDate){
				$whereSql 	 .= " AND P.payment_date BETWEEN '".$startDate."' AND '".$endDate."'";
				
			}else{ // Then we don't need a BETWEEN clause :)
				$whereSql .= " AND P.payment_date = '".$startDate."'";
			}			
		}
		$whereSql_2 .= $whereSql;
	}
	...
	$query	=	"SELECT count(*) AS total	
				...
				WHERE P.payment_error_code <> '' AND P.payment_error_code <> '00000' ".$joinquery;
	$totals = $GLOBALS['db']->getRow($query, DB_FETCHMODE_ASSOC);
	$query	= "SELECT count(*) AS total_condition FROM payment AS P
				INNER JOIN user_master AS UM ON UM.user_id = P.payment_userid 
	...
	if($total_condition['total_condition']>0){
		...
		$sql = "SELECT UM.user_fname, UM.user_lname, P.payment_userid, 
					COUNT(P.payment_userid) AS count, 
					($cent_unit * COUNT(P.payment_userid)) AS percent
					FROM payment AS P 
					INNER JOIN user_master AS UM ON UM.user_id = P.payment_userid 
					WHERE P.payment_error_code <> '' AND P.payment_error_code <> '00000' 
					$whereSql $joinquery
					GROUP BY P.payment_userid 
					ORDER BY COUNT(P.payment_userid) DESC, UM.user_fname ASC";
		//$excel_query	= ;
	//*************************** Countrywice report for the users starts here ****************************
		$paging_query	= "SELECT count(DISTINCT(P.payment_userid)) 
					FROM payment AS P 
					INNER JOIN user_master AS UM ON UM.user_id = P.payment_userid 
					WHERE P.payment_error_code <> '' AND P.payment_error_code <> '00000' 
					$whereSql $joinquery";
	...
	////////////////////////////////////Pagination ends here/////////////////////////////////////////
		if($totalRecs > 0){
			for($i=0, $max=sizeof($result); $i<$max; $i++){
				$user_ids_arr[]	= $result[$i]['payment_userid'];
			}
			$user_ids	= implode(',', $user_ids_arr);
			$sql1	=	"SELECT P.payment_userid, P.payment_error_code, 
							DATE_FORMAT(P.payment_date, '%D %M %Y') AS payment_date 
						FROM payment AS P 
						WHERE P.payment_userid IN ($user_ids)
						AND P.payment_error_code <> '' AND P.payment_error_code <> '00000' 
						$whereSql_2
						ORDER BY P.payment_date Desc";
			$rs		=	$GLOBALS['db']->query($sql1);
			while($row = $rs->fetchRow(DB_FETCHMODE_ASSOC)){
				$programs[$row['payment_userid']][]	= $row;
			}
			$rs->free();
		}
	}
              ...
              <tr> 
				...
				<TABLE cellSpacing=0 cellPadding=0 border=0 align="center">
				  ...
				  <tr>
				    ...
				    <td width="117" height="27" align="center" valign="bottom" class="sectionHeading"><a href="excel_transaction_issues.php?inf=<?php echo(base64_encode($whereSql).'&fromLimit='.$fromLimit.'&toLimit'.$toLimit);
					?>"><img src="../images/sports/english/download.gif" style="float:right" border="0"></a></td>
				  </tr>
...
<select name="toY">
  <?
				  ...
				  <TABLE class="listTableStyle" cellSpacing=1 cellPadding=2 width="553">
                    <TBODY>
                      <TR class="tableHeaderColor">
                        ...
                        <TD width="32%" align="center" >Transaction Issues </TD>
			...
			?><tr  <? if(($key%2) ==1){?> bgcolor="#FFFFFF" <? } ?> >
                              ...
                              <TD width="37%" height="19" align="left" style="padding-left:10px;"><a href="javascript:;" onMouseOver="myFunction('<?=$val['payment_userid']?>')" onClick="return overlib(INARRAY, <?=$val['payment_userid']?>, CAPTION, '<? echo addslashes($val['user_fname'])." ".addslashes($val['user_lname']).' - '.$val['count'];?>');" ><? echo $val['user_fname']." ".$val['user_lname'];?></a></TD>
    ...
    <TD vAlign=top align=left class="topBarColor" colspan="3">&nbsp;</TD>
        ...
        <?php 
			...
			// Below code creates hiiden DIVs for each user.
			for($i=0, $max=sizeof($result);$i<$max;$i++){
		?><div id="<?=$result[$i]['payment_userid']?>" class="hidden_div" >
			<div class="popup_box">
				<table width="100%">
					<tr>
						<td width="70%" class="boldC">Error Code</td>
						<td width="30%" class="boldC">Transaction Date</td>
					</tr>
				<?php
					for($k=0, $max3=sizeof($programs[$result[$i]['payment_userid']]); $k<$max3; $k++){
				?><tr>
					<td class="tblbackgnd" colspan="2">
					<table width="100%" cellspacing="0" cellpadding="1">
					  <tr <? if(($k%2) ==1){?> bgcolor="#FFFFFF" <? } ?>>
						<td width="70%"><? 
						echo $programs[$result[$i]['payment_userid']][$k]['payment_error_code'].': ';
						echo $paymentObj->getErrorCodeMapping($programs[$result[$i]['payment_userid']][$k]['payment_error_code']);
						?></td>
						<td width="30%"><?=$programs[$result[$i]['payment_userid']][$k]['payment_date']?></td>
					  </tr>
					</table>
					</td>
				</tr>
