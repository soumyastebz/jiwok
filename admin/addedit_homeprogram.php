...
$body	=	$editpgm['0']['program'];
//print_r($editpgm);
$combo	=	$objService->get_combo_arr('category',$getAllTrainCats,'flex_id','program_title',$fid,$params="style=width:600px;");
	/*
	...
	*/
	...
	if($_POST['add']){
		...
		if(count($errorMsg)==0)	{
			...
			$nextId = $objService->_insertMaster(addslashes($_POST['desc']),$_POST['category'],$langval,$_POST['user_image']);
			header("Location:list_homeprogram.php?status=success_add&langId=".$_REQUEST['langId']."&pageNo=".$_REQUEST['pageNo']."&maxrows=".$_REQUEST['maxrows']."&type=".$_REQUEST['type']."&field=".$_REQUEST['field']."&keyword=".$_REQUEST['keyword']);
		}
	}
	...
	if($_POST['update']){
		...
		//	reset($languageArray);
		//	while(list($key,$value) = each($languageArray)){
					echo  $query = "SELECT  count(*) as dataCount FROM home_program WHERE manager_id =".$_REQUEST['slideId']." AND language_id=".$langval.""; 
$result = $GLOBALS['db']->getAll($query, DB_FETCHMODE_OBJECT);
					...
					if(DB::isError($result)) 
						...
						if($coundRecs >= 1){
                           ...
                           $_POST['user_image']    = $fileName;
							...
							$status	= $langval;
							$elmts	= array("program" => $desc,"flex_id" => $category,"language_id" => $status,"image"=> $_POST['user_image']);
		...
		//	}
			...
			header("Location:list_homeprogram.php?status=success_update&langId=".$_REQUEST['langId']."&pageNo=".$_REQUEST['pageNo']."&maxrows=".$_REQUEST['maxrows']."&type=".$_REQUEST['type']."&field=".$_REQUEST['field']."&keyword=".$_REQUEST['keyword']);
                            ...
                            <TD valign="top">
								  ...
								  <tr>
									...
									<TD align="left">
										...
										<table height="50" class="topActions"><tr><td valign="middle" width="50"><a href="list_homeprogram.php?pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$_REQUEST['keyword']?>"><img src="images/list.gif" border="0" alt="Listing Record">&nbsp;List   </a></td>
...
<?php } ?>
					 ...
					 <td width="79%">
						...
						<tr><td width="25%" align="left">
						  Training List<?php echo REQUIRED;?>:&nbsp;						</td>
						<td width="79%">
