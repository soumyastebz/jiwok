...
INNER JOIN payment_dummy AS PS ON PP.user_id = PS.payment_userid
INNER JOIN user_master AS UM ON UM.user_id = PP.user_id WHERE PS.payment_status='1' AND PS.version='New'  $whereSql";

	$totals = $GLOBALS['db']->getRow($query, DB_FETCHMODE_ASSOC);
	/*$query	= "SELECT count(*) AS total_condition	FROM payment_dummy AS PS				
				INNER JOIN user_master AS UM ON UM.user_id = PS.payment_userid
				INNER JOIN payment_paybox AS PP ON PP.user_id = PS.payment_userid = UM.user_id
	...
	if($total_condition['total_condition']>0){
		...
		$sql = "SELECT UM.user_fname, UM.user_lname,UM.user_email,PS.payment_amount,PS.payment_currency,PS.payment_expdate,PS.payment_date, UM.user_id 					
					FROM payment_paybox AS PP										
					INNER JOIN payment_dummy AS PS ON PP.user_id = PS.payment_userid
					INNER JOIN user_master AS UM ON UM.user_id = PP.user_id WHERE PS.payment_status='1' AND PS.version='New'
					$whereSql
					";
...
<select name="toY">
  <?
				  ...
				  <TABLE class="listTableStyle" cellSpacing=1 cellPadding=2 width="553">
                    <TBODY>
                      <TR class="tableHeaderColor">
                        ...
                        <TD width="16%" align="center" >Exp.Date</TD>
                      </TR>
                      <?php if($errMsg != ""){?>
			...
			?><tr  <? if(($key%2) ==1){?> bgcolor="#FFFFFF" <? } ?> >
                              ...
                              <TD width="20%" height="19" align="left" style="padding-left:10px;"><a href="javascript:;" onMouseOver="myFunction('<?=$val['user_id']?>')" onClick="return overlib(INARRAY, <?=$val['user_id']?>, CAPTION, '<? echo addslashes($val['user_fname'])." ".addslashes($val['user_lname']).' - '.$val['count'];?>');" ><? echo $val['user_fname']." ".$val['user_lname'];?></a></TD>
                              <TD width="24%" height="19" align="left" style="padding-left:10px;"><?php echo $val['user_email'];?></TD>
                              <TD width="8%" align="center" style="padding-left:10px;"><?php echo $val['payment_amount']?></TD>
                              <TD width="11%" align="center" style="padding-left:10px;"><?php echo $val['payment_currency'];?></TD>
                              <TD width="16%" height="19" align="center" style="padding-left:10px;"><?php echo $val['payment_date'];?></TD>
                              <TD width="15%" align="center" style="padding-left:10px;"><?php echo $val['payment_expdate'];?></TD>
                            </tr>
			...
			?>							
					...
					<tr><?php if($noOfPage > 1) { ?>
						...
						<a href="report_newPayment.php?pageNo=1&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>">
						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>
						<a href="report_newPayment.php?pageNo=<?=$prev?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>">
						...
						<select name="pageNo" class="paragraph"  onChange="form.submit()">
							...
							?>
							 ...
							 of <?=$noOfPage?>]
							 <a href="report_newPayment.php?pageNo=<?=$next?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>">
							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>
							<a href="report_newPayment.php?pageNo=<?=$noOfPage?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>">
    ...
    <TD vAlign=top align=left class="topBarColor" colspan="3">&nbsp;</TD>
		...
		?><div id="<?=$result[$i]['user_id']?>" class="hidden_div" >
			<div class="popup_box">
				<table width="100%">
					<tr>
						<td width="70%" class="boldC">Test</td>
						<td width="30%" class="boldC">Test</td>
					</tr>
				<?php
