<?php
	...
	$query = "SELECT count(*) as max,PC.*,PD.payment_currency,PD.payment_amount,UM.user_email FROM payment_transactions PC LEFT JOIN payment PD ON PC.payment_id = PD.payment_id LEFT JOIN user_master UM on UM.user_id = PD.payment_userid WHERE PC.status = 'PAID' AND PD.payment_status = 1";
	...
	if($_REQUEST['pageNo']){
		...
		$result = $payObj->_showPage($totalRecs,$_REQUEST['pageNo'],$_REQUEST['maxrows'],$field,$type,$searchQuery);
	}
	else{
