...
//	error_reporting(0);
	...
	*/
	...
	$heading = "Static Download Report";
	...
	*/
	...
	if($_POST['daterange'] == 1){
		...
		switch($_POST['dropdown1']){
			case 'today':
				$whereSql .= " AND DATE(SD.download_date) = '".$today."'";
			...
			case 'yest':
				...
				$whereSql .= " AND DATE(SD.download_date) = '".$yesterday."'";
			...
			case 'last7':
				...
				$whereSql .= " AND DATE(SD.download_date) BETWEEN '".$requiredDate."' AND '".$today."'";
			...
			case 'thismonth':	
				...
				$whereSql 	 .= " AND MONTH(SD.download_date) = '".$thisMonth."'  AND YEAR(SD.download_date) = '".$thisYear."'";
			...
			case 'lastmonth':	
				...
				$whereSql 	 .= " AND MONTH(SD.download_date) = '".$lasMonth."'  AND YEAR(SD.download_date) = '".$lasYear."'";
				...
				$whereSql .= " AND YEAR(SD.download_date) = '".$lasYear."'";
		...
		if(count($errorMsg) == 0){
			if($startDate != $endDate){
				$whereSql 	 .= " AND DATE(SD.download_date) BETWEEN '".$startDate."' AND '".$endDate."'";
				
			}else{ // Then we don't need a BETWEEN clause :)
				$whereSql .= " AND DATE(SD.download_date) = '".$startDate."'";
			}			
		}
		$whereSql_2 .= $whereSql;
	}
	...
	if(isset($_POST['report']) && ( $_POST['report']=='paid' || $_POST['report']=='free')){
		if(createPaidUsers()===true){
			if($_POST['report']=='paid'){
				$payment_qry	=	" INNER JOIN paid_users_temp AS PA ON PA.payment_userid = SD.user_id ";
			}elseif($_POST['report']=='free'){
				$whereSql	.=	" AND PA.payment_userid IS NULL ";
				$payment_qry	=	" LEFT JOIN paid_users_temp AS PA ON PA.payment_userid = SD.user_id ";
			}
		}
	}
		
	$query	= "SELECT count(*) AS total	
				FROM static_download_log AS SD
				INNER JOIN user_master AS UM ON UM.user_id = SD.user_id";
	$totals = $GLOBALS['db']->getRow($query, DB_FETCHMODE_ASSOC);
	$query	= "SELECT count(*) AS total_condition FROM static_download_log AS SD
				$payment_qry
				INNER JOIN user_master AS UM ON UM.user_id = SD.user_id
				$whereSql";
	//die($query);
	...
	if($total_condition['total_condition']>0){
		...
		$sql = "SELECT UM.user_fname, UM.user_lname, SD.user_id, 
					COUNT(SD.user_id) AS count, 
					($cent_unit * COUNT(SD.user_id)) AS percent
					FROM static_download_log AS SD 
					$payment_qry
					INNER JOIN user_master AS UM ON UM.user_id = SD.user_id
					$whereSql
					GROUP BY SD.user_id ORDER BY COUNT(SD.user_id) DESC, UM.user_fname ASC";
	//die($sql);
	...
	//$sql = $sqlStart.$fromSql.$whereSql." GROUP BY user_master.user_id ORDER BY user_master.user_doj DESC";
		...
		$paging_query	= "SELECT count(DISTINCT(SD.user_id)) 
					FROM static_download_log AS SD 
					$payment_qry
					INNER JOIN user_master AS UM ON UM.user_id = SD.user_id
					$whereSql";
...
//////////////////////////////// PAGINATION //////////////////////////////////
	...
	////////////////////////////////////Pagination ends here/////////////////////////////////////////
		...
		if($totalRecs > 0){
			...
			$sql1	=	"SELECT SD.user_id, SD.workout_flex_id, WD.workout_title, 
							DATE_FORMAT(SD.download_date, '%D %M %Y') AS download_date
						FROM static_download_log AS SD 
						LEFT JOIN workout_details AS WD ON WD.workout_flex_id = SD.workout_flex_id
						WHERE SD.user_id IN ($user_ids)
						$whereSql_2
						GROUP BY SD.download_id";
			
			
			$sql_count_tot = "SELECT count(*) AS total_count	
				FROM static_download_log AS SD
				INNER JOIN user_master AS UM ON UM.user_id = SD.user_id WHERE SD.user_id IN ($user_ids)
				$whereSql_2";
				
			$rs_cnt		=	$GLOBALS['db']->query($sql_count_tot);
			$result_count = $rs_cnt->fetchRow(DB_FETCHMODE_ASSOC);
              ...
              <tr> 
				...
				<TABLE cellSpacing=0 cellPadding=0 border=0 align="center">
				  ...
				  <tr>
				    ...
				    <td width="117" height="27" align="center" valign="bottom" class="sectionHeading"><a href="excel_work.php?inf=<? echo base64_encode($sql);?>"><img src="../images/sports/english/download.gif" style="float:right" border="0"></a></td>
				  </tr>
  ...
  <?
				  ...
				  <tr style="padding-top:10px;">
					...
					<select id="report" name="report" onChange="change();">
					   <option value="all" <? if(!isset($_REQUEST['report']) || $_REQUEST['report']=='all') echo 'selected="selected"'?> >Whole</option>
                  ...
                  <TABLE cellSpacing=1 cellPadding=2 width="553">
                    <TBODY>
					<TR >
                        <TD width="50%" align="left" class="sectionHeading">Total Downloads :</TD>
                        <TD width="50%" align="left" class="sectionHeading"><?=$result_count['total_count']?></TD>
                      </TR>
                      <TR >
                        <TD width="50%" align="left" class="sectionHeading">Percentage of Downloads :</TD>
				  ...
				  <TABLE class="listTableStyle" cellSpacing=1 cellPadding=2 width="553">
                    <TBODY>
                      <TR class="tableHeaderColor">
                        ...
                        <TD width="32%" align="center" > No of Downloads </TD>
			...
			?>							
						...
						<a href="report_workouts.php?pageNo=1&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>">
						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>
						<a href="report_workouts.php?pageNo=<?=$prev?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>">
						...
						?>
							 ...
							 of <?=$noOfPage?>]
							 <a href="report_workouts.php?pageNo=<?=$next?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>">
							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>
							<a href="report_workouts.php?pageNo=<?=$noOfPage?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>">
			...
			<div class="popup_box">
				<table width="100%">
					<tr>
						...
						<td width="30%" class="boldC">Generation Date</td>
					</tr>
				<?php
				...
				?><tr>
					...
					<table width="100%" cellspacing="0" cellpadding="1">
					  <tr <? if(($k%2) ==1){?> bgcolor="#FFFFFF" <? } ?>>
						<td width="70%"><?=$programs[$result[$i]['user_id']][$k]['workout_title']?></td>
						<td width="30%"><?=$programs[$result[$i]['user_id']][$k]['download_date']?></td>
					  </tr>
					</table>
					</td>
				</tr>
