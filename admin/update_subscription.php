				...
				if(!$memObj->updatePaymentstripe($pay_result,$expiry_date))
								{
									...
									$error_Msg  = 'Changing payment failed';
								}
  //echo "<pre/>";echo "pp";print_r($_SERVER);exit;
								$redirect =$_SERVER['HTTP_REFERER'];
								header('Location:'.$redirect."&error_Msg=".$error_Msg);exit;
			}
		...
		elseif (isset($_REQUEST['stripeUnsubscribe']))
		{
				 ...
				 $sqlQry		 =	"SELECT `id`,`customer_id`,`subsciption_id` FROM `stripe_payment` where `user_id`=".$userId." order by id desc ";
				$res		 =	$GLOBALS['db']->getAll($sqlQry,DB_FETCHMODE_ASSOC);
				try
				{
				$cu = \Stripe\Customer::retrieve($res[0]['customer_id']);				
				$resq = $cu->subscriptions->retrieve($res[0]['subsciption_id'])->cancel();
				...
				//if($userId =='281850')
				 if($resq->status =='canceled')				 
				 {
					...
					$rslt=$dbObj->_updateRecord("stripe_payment",$elmts,"id = ".$res[0]['id']);
					...
					$rslt_new=$dbObj->_updateRecord("stripe_auto_renewal",$elmts_new,"pp_id = ".$res[0]['id']);
				...
				$confMsg = "Successfully Unsubscribed";		
							...
							$body = $e->getJsonBody();
    							$err  = $body['error'];								
							   $paymentDetails	=	base64_encode(serialize($err));
								$temparray = array();
                            ...
                            <TD>
								  ...
								  <tr>
									...
									<tr>
										...
										<td align="center"><? echo $error_Msg; ?></td>
				  ...
				  <TABLE class="paragraph2" cellSpacing=0 cellPadding=2 width="80%" align="center">
				   ...
				   <TBODY> 				   
					...
					if((mysql_num_rows($resultUnSub)>0) || (mysql_num_rows($resultUnSubstripe)>0)){
						  ...
						  if(mysql_num_rows($resultUnSubstripe)>0)
						   {
							   $unSubData	=	mysql_fetch_array($resultUnSubstripe);
							    $unSubData['version']	=	" from stripe";
						   }
						  elseif(mysql_num_rows($resultUnSub)>0)
						    {
							   $unSubData	=	mysql_fetch_array($resultUnSub);
							   $unSubData['version']	=	" from paybox";
							}
					...
					?> 
					   <tr>
						...
						<td>: <?php echo $unSubData['unsubscribed_date']."  ".$unSubData['version']; ?></td>
					...
					if($result['user_unsubscribed'] == 2){?>
					 ...
					 <tr>
                       ...
                       <?php if(($pay_result['version'] == 'stripe' ) || ($pay_result['version'] == 'mobile_stripe' )) //need to chek android paymnets too 
					   		{
								
								if (mysql_num_rows($resultDatastripe)>0)
								 {
									 $nxtPaymentData	=	mysql_fetch_array($resultDatastripe);	//  echo "222";echo "<pre>";print_r($nxtPaymentData);exit;	
										if($nxtPaymentData["status"]=='VALID')
										{
												...
												{
													$nxtPaymentData['nextplan']    = $nxtPaymentData["plan_name"]."(With discount code)"." via stripe";
													$pieces 						= explode("##", $nxtPaymentData['discount_amount']);																
													$nxtPaymentData["payment_amount"]	=	trim($pieces[0]);
												}
												else
												{
													$nxtPaymentData['nextplan'] = $nxtPaymentData["plan_name"]." via stripe ";
													$nxtPaymentData["payment_amount"]	=	$nxtPaymentData["plan_amount"];		
												}
										}
										else
										{
											$nxtPaymentData["payment_amount"] = $nxtPaymentData["status"];
											$nxtPaymentData['nextplan']= $nxtPaymentData["status"];
										}
										
									
								 }
					   }
					   elseif($pay_result['version'] == 'IAP')
					   {
						  if (mysql_num_rows($resultDataIap)>0)
						 {
							 $nxtPaymentData	=	mysql_fetch_assoc($resultDataIap);	
							  $nxtPaymentData['nextplan'] 	=	"Pass ".$nxtPaymentData["payment_duration"]." mois via Iphone";
							 $nxtPaymentData['payment_amount'] 	=    $nxtPaymentData["payment_amount"]." ".$nxtPaymentData["payment_currency"];
						 }
					   }
					   else
					   {
								 ...
								 if($nxtPaymentData["status"]=='VALID')
									{
										if(($nxtPaymentData['discount_amount']	!=	0)	&&	($nxtPaymentData['plan_id']	==	1))
										{
											$nxtPaymentData['nextplan'] = $nxtPaymentData["plan_name"]."(With discount code)"." via ".$nxtPaymentData['via'];
											$pieces 					= explode("##", $nxtPaymentData['discount_amount']);																
											$nxtPaymentData["payment_amount"]	=	trim($pieces[0]);
										}
										else
										{
											$nxtPaymentData['nextplan'] = $nxtPaymentData["plan_name"]." via ".$nxtPaymentData['via'];
											$nxtPaymentData["payment_amount"]	=	$nxtPaymentData["plan_amount"];	
										}
									}
									else
					...
					?>
					   ...
					   <td>: <?php echo $nxtPaymentData['nextplan'] 
					   				/*if (mysql_num_rows($resultDataIap)>0)
									{
										echo  $nxtPaymentData['via'];
									}
					 ...
					 <tr>
					   ...
					   <td>: <?php echo $nxtPaymentData["payment_amount"];
					   				/*if (mysql_num_rows($resultDataIap)>0)
									{
					...
					<?php } ?>
					 ...
					 if((($result['user_unsubscribed'] == 2 || !$oldPaid)	&&	(count($payboxStatus)	==	0)) && (($result['user_unsubscribed'] == 2 || !$oldPaid)	&&	(count($payboxStatusstripe)	==	0))){ 
					   ...
					   ?><input type="button" name="subscribe" value="Subscribe User"  onClick="return false"><?php } 
                       ...
                       <tr>
                         ...
                         <td align="left"><strong>Refund</strong>
							...
							else if(count($refundstripe)	>	0)
							{
								...
								{
									//echo "<pre>";print_r($paymentstripe[0]['balance']);exit;
									$Paymentcur 	= 	"SELECT payment_amount,payment_currency,version FROM payment WHERE payment_id=".$refund['payment_id'];
	                                 $Paymentcurresult = $GLOBALS['db']->getRow($Paymentcur, DB_FETCHMODE_ASSOC);
									 $amountDescription	=	($Paymentcurresult['payment_amount']-$paymentstripe[0]['balance'])." ".$Paymentcurresult['payment_currency'];
									 $method		= $Paymentcurresult['version'];	
									?>
                                	<tr>
                                    	<td width="16%">&nbsp;<?=$key+1?></td>
