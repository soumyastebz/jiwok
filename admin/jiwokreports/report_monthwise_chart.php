...
error_reporting(1);
	...
	{
		$cntry = $_POST['user_country'];
		$countryCondn = " AND user_country = ".$_POST['user_country']." ";
	}
	else
	{
		$countryCondn = " AND user_country = 73 ";
		$cntry =73;
	}	
	if($_POST['num_months']!="")
	{
	...
	*/
	...
	//$reportQuery = "SELECT Brand,num_months,SUM(Count) AS Count FROM(".$reportQuery.")AS t4 GROUP BY t4.Brand,t4.num_months";
	
	$reportQuery = "SELECT Brand,GROUP_CONCAT(num_months) AS num_months,GROUP_CONCAT(Count) AS Count FROM(".$reportQuery.") AS t5 GROUP BY Brand ORDER BY t5.Brand";
	...
	{		
		${"arrayMonth_{$i}"} = explode(',',$row['num_months']);
		...
		{
			${"data_{$i}"} .= " data".$i.".setValue(".$j.", 0, '".${"arrayMonth_{$i}"}[$j]." Months');data".$i.".setValue(".$j.", 1, ".$arrayCount.");";
			$j++;
		}
...
<script type="text/javascript" src="https://www.google.com/jsapi"></script>
    ...
    <?php for($i = 0; $i< $totalRows;$i++){?>	
	 ...
	 /* google.load("visualization", "1", {packages:["corechart"]});
	  ...
	  {
      	google.setOnLoadCallback(drawChart('chart_div',i));
	  }
      function drawChart(div,i) {
        var data = new google.visualization.DataTable();    
	  
        var chart = new google.visualization.PieChart(document.getElementById(div+i));
        chart.draw(data, {width: 775, height: 625, title: 'My Daily Activities'});
      }*/
...
<span style="vertical-align:top">
                                        ...
                                        <tr>
                                          <td width="182" align="left" valign="top" style="padding-left:20px;padding-top:10px;">&nbsp;<strong>Brand</strong>&nbsp;&nbsp;</td>
                                          <td width="161"  style="padding-top:10px;" valign="top">
