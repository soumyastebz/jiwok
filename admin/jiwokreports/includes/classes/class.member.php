...
class Member extends DbAction{
	...
	*/
	...
	/*
	   ...
	   $Getemail   = mysql_query("select user_email from user_master where user_id=".$NewArray[$numuser]);
		...
		$Ckadmin    = mysql_fetch_assoc($ForumAdmin); 
		   ...
		   $ResTicketid = mysql_fetch_assoc($SqlTicketid); 
...

		   		{
			   		...
			   		$this->_deleteData('ticket_ticket_replies', 'reporter_id='.$ResTicketid['client_id']);  // delete ticket reply

			   		$this->_deleteData('ticket_tickets', 'client_id='.$ResTicketid['client_id']); // delete ticket

			   		$this->_deleteData('ticket_clients', 'client_id='.$ResTicketid['client_id']); // delete user
...

	{
		...
		foreach($res_mail as $key=>$value)

			{
				...
				@mail($uemail,$subject,$msg,$headers);
