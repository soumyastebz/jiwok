...
error_reporting(1);
	...
	$heading = "Subscribers Reports : Plan Wise";
	$todayDetails = getdate();
	
	$_POST['report_criteria']  =3;
	...
	*/
	...
	$reportQuery = sprintf($reportQuery,$frM,$frY,$toM,$toY,$countryCondn,$genderCondn,$originCondn,$discCondn,$ageCondn1,$ageCondn2,$brandCondn,$typeCondn,$programCondn,$languageCondn);	
	...
	{
		$resultArray[$i]['Brand'] = $row['Brand'];
		$resultArray[$i]['cat'] = $row['Plan'];
	...
	while($row = mysql_fetch_array($result1,MYSQL_ASSOC))
	{
		...
		$cmpArray[$i]['cat'] = '1Euro';
		$cmpArray[$i]['catnb'] = 1;
		$i++;
		$cmpArray[$i]['Brand'] = $row['brand'];
		$cmpArray[$i]['cat'] = 'Giftcode';
		$cmpArray[$i]['catnb'] = 2;
		$i++;
		$cmpArray[$i]['Brand'] = $row['brand'];
		$cmpArray[$i]['cat'] = 'FWT';
	...
	foreach($cmpArray AS $cmpArrays)
	{
		...
		foreach($resultArray AS $resultArrays)
		{
			if((strtolower($cmpArrays['Brand']) == strtolower($resultArrays['Brand'])) && (strtolower($cmpArrays['cat']) == strtolower($resultArrays['cat'])))
					{						
						$dat .= 	"data.setValue(".(int)($i/4).", ".$cmpArrays['catnb'].", ".$resultArrays['Count'].");";
						$j ++;						
					}
		}
		if($j ==0)
				{
					$dat .= 	"data.setValue(".(int)($i/4).", ".$cmpArrays['catnb'].",0);";
				}
				$i++;	
	}
...
google.load("visualization", "1", {packages:["corechart"]});
      ...
      function drawChart() {
        ...
        data.addColumn('number', '1Euro');
		data.addColumn('number', 'Giftcode');
		data.addColumn('number', ' FWT');
		data.addColumn('number', ' 7.9/9.9 trans');
                        ...
                        <td width="11" height="9"><img src="images/top_right.jpg" width="9" height="9"></td>
                          ...
                          <tr> 
                            ...
                            <td valign="top"> 
                                ...
                                <tr>
                                          <td height="6" colspan="4" align="center" valign="bottom" class="sectionHeading" style="font-size:16px; padding-top:10px;">Planwise Reports</td>
                                 </tr>                                  
                                    <TR> 
                                      <TD valign='top' bgColor='white'>
                                      <form name="reportFrm" action="report_planwise_chart.php" method="post" enctype="multipart/form-data">
                           ...
                           ?>
                                          ...
                                          <td width="89" align="left" valign="top" style="padding-left:10px;"><strong>Age</strong></td>
                                          <td width="197" valign="top" ><input type="text" name="user_fromage" id="user_fromage" size="5" value="<?=trim($_POST['user_fromage'])?>" />&nbsp;- &nbsp;<input type="text" name="user_toage" id="user_toage" size="5" value="<?=trim($_POST['user_toage'])?>"/></td>
                                        </tr>
                                        <tr >
										 ...
										 <!-- <select id="user_origin" name="user_origin" style="font-size:11px;" >
                                          ...
                                          </select>--></td><td width="126" align="left" valign="top" style="padding-left:10px;"><strong>Language</strong></td>
                                        ...
                                        <tr>
                                          <td width="246" align="left" valign="top" style="padding-left:20px;padding-top:10px;">&nbsp;<strong>Brand</strong>&nbsp;&nbsp;</td>
                                          <td width="218"  style="padding-top:10px;" valign="top">
                                          <span style="vertical-align:top">
