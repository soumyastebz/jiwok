...
error_reporting(1);
	...
	*/
	...
	mysql_query(trim('SET SESSION group_concat_max_len = 100000')) or die(mysql_error());	
	$reportQuery = "SELECT Brand,GROUP_CONCAT(program_title SEPARATOR '##**') AS program_title,GROUP_CONCAT(Count) AS Count FROM(".$reportQuery.") AS t5 GROUP BY t5.Brand ORDER BY t5.Brand";	
	...
	{			
		${"arrayTitle_{$i}"} = explode('##**',$row['program_title']);		
		${"arrayCount_{$i}"} = explode(',',$row['Count']);
		${"brand_{$i}"}		  = $row['Brand'];
		${"data_{$i}"}          = " var data".$i." = new google.visualization.DataTable();data".$i.".addColumn('string', 'Months');data".$i.".addColumn('number', 'Count');data".$i.".addRows(".count(${"arrayCount_{$i}"}).");";
		$j = 0;
		foreach(${"arrayCount_{$i}"} AS $arrayCount)	
		{
			${"data_{$i}"} .= " data".$i.".setValue(".$j.", 0, '".addslashes(${"arrayTitle_{$i}"}[$j])."');data".$i.".setValue(".$j.", 1, ".$arrayCount.");";
			$j++;
		}
		//echo count($row['num_months']);
		$i++;
	}
...
<script type="text/javascript" src="https://www.google.com/jsapi"></script>
 ...
 google.load("visualization", "1", {packages:["corechart"]});
    <?php for($i = 0; $i< $totalRows;$i++){?>	
      ...
      <?php echo ${"data_{$i}"};?>
        var chart<?php echo $i;?> = new google.visualization.PieChart(document.getElementById('chart_div<?php echo $i;?>'));
        chart<?php echo $i;?>.draw(data<?php echo $i;?>, {width: 775, height: 625, title: '<?php echo ${"brand_{$i}"}." Brand";?>'});		
      }
	  <?php }?>  
</script>
                        ...
                        <td width="11" height="9"><img src="images/top_right.jpg" width="9" height="9"></td>
                          ...
                          <tr> 
                            ...
                            <td valign="top"> 
                                ...
                                <tr>
                                          <td height="6" colspan="4" align="center" valign="bottom" class="sectionHeading" style="font-size:16px; padding-top:10px;">Training Program Reports</td>
                                 </tr>
                                    
                                    <TR> 
                                      <TD valign='top' bgColor='white'>
                                      <form name="reportFrm" action="report_training_chart.php" method="post" enctype="multipart/form-data" onSubmit="return dateValidation();">
                           ...
                           ?>
                                          ...
                                          <td width="102" align="left" valign="top" style="padding-left:10px;"><strong>Age</strong></td>
                                          <td width="164" valign="top" ><input type="text" name="user_fromage" id="user_fromage" size="5" value="<?=trim($_POST['user_fromage'])?>" />&nbsp;- &nbsp;<input type="text" name="user_toage" id="user_toage" size="5" value="<?=trim($_POST['user_toage'])?>"/></td>
                                        </tr>
                                        <tr >
...
<span style="vertical-align:top">
                                        ...
                                        <tr>
                                          <td width="206" align="left" valign="top" style="padding-left:20px;padding-top:10px;">&nbsp;<strong>Brand</strong>&nbsp;&nbsp;</td>
                                          <td width="278"  style="padding-top:10px;" valign="top">
                                          <span style="vertical-align:top">
                                       ...
                                       <tr><td colspan="8" height="30">&nbsp;</td>
                                        ...
                                        <tr >
                                        <td width="206" style="padding-left:20px; padding-top:30px;"><strong>Filter Date</strong></td>
