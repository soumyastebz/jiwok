...
error_reporting(1);
	...
	*/
	...
	while($row = mysql_fetch_array($result1,MYSQL_ASSOC))
	{
		...
		$i++;
		$cmpArray[$i]['Brand'] = $row['brand'];
		$cmpArray[$i]['cat'] = 'Existing';
		$cmpArray[$i]['catnb'] = 2;
		$i++;
		$cmpArray[$i]['Brand'] = $row['brand'];
		$cmpArray[$i]['cat'] = 'other';
		$cmpArray[$i]['catnb'] = 3;		
	...
	foreach($cmpArray AS $cmpArrays)
	{
		...
		foreach($resultArray AS $resultArrays)
		{
			if((strtolower($cmpArrays['Brand']) == strtolower($resultArrays['Brand'])) && (strtolower($cmpArrays['cat']) == strtolower($resultArrays['cat'])))
					{						
						$dat .= 	"data.setValue(".(int)($i/3).", ".$cmpArrays['catnb'].", ".$resultArrays['Count'].");";
						$j ++;						
					}
		}
		if($j ==0)
				{
					$dat .= 	"data.setValue(".(int)($i/3).", ".$cmpArrays['catnb'].",0);";
				}
				$i++;	
	}		
                            ...
                            <td valign="top"> 
                                ...
                                <tr>
                                    ...
                                    <TR> 
                                      ...
                                      <form name="reportFrm" action="report_register_chart.php" method="post" enctype="multipart/form-data" onSubmit="return dateValidation();">
