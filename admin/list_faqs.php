...
/**************************************************************************** 
   ...
   DESCRIPTION::::>>>>
   This  code userd to list the all faqs .
   Admin can add/edit/delete the faqs .. 
*****************************************************************************/
	...
	*/
	...
	if($_REQUEST['field']){
		$field = "faq_manager.".$_REQUEST['field'];
		$type = $_REQUEST['type'];
	}else{
		$field = "faq_id";
		$type = "ASC";
	...
	if(trim($_REQUEST['keyword'])){
	 $cleanData	=	str_replace("'",'\\\\\\\\\'',trim($_REQUEST['keyword']));
		...
		$errMsg = "Special characters are not allowed";
			...
			$searchQuery	=	" AND manager_question like '%".$cleanData."%'";}
	...
	if($_REQUEST['action'] == "delete"){
		$id		 = $_REQUEST['faqId'];
		$result	 = $objFaq->_deleteFaq($id);
		$confMsg = "Successfully Deleted";
	}	
	
	$totalRecs = $objFaq->_getTotalCount($searchQuery,$lanId);
	if($totalRecs <= 0)
		$errMsg = "No Records";
	...
	if($_REQUEST['pageNo']){
		...
		$result =  $objFaq->_showPage($totalRecs,$_REQUEST['pageNo'],$_REQUEST['maxrows'],$field,$type,$searchQuery);
	}
	else{
