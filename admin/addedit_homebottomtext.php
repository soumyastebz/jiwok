	...
	*/
	...
	if($_POST['update']){
		...
		if(count($errorMsg)==0)	{
			reset($languageArray);
			while(list($key,$value) = each($languageArray)){
					 $query = "SELECT  count(*) as dataCount FROM hometext_bottom_manager WHERE hometext_bottom_master_id =".$_REQUEST['slideId']." AND language_id=".$key."";
$result = $GLOBALS['db']->getAll($query, DB_FETCHMODE_OBJECT);
					...
					if(DB::isError($result)) 
						...
						if($coundRecs >= 1){
							$title	= $_POST['service_content_'.$key];
							$status	= $_POST['service_status'];
							$elmts	= array("text_content" => $title,"text_status" => $status);
			...
			header("Location:list_homebottomtext.php?status=success_update&langId=".$_REQUEST['langId']."&pageNo=".$_REQUEST['pageNo']."&maxrows=".$_REQUEST['maxrows']."&type=".$_REQUEST['type']."&field=".$_REQUEST['field']."&keyword=".$_REQUEST['keyword']);
...
<tr> 
              ...
              <tr> 
				...
				<form name="serviceform" action="addedit_homebottomtext.php" method="post" onSubmit="return formChecking()">
                            ...
                            <TD valign="top">
								  ...
								  <tr>
									...
									<TD align="left">
										...
										<table height="50" class="topActions"><tr><td valign="middle" width="50"><a href="list_homebottomtext.php?pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$_REQUEST['keyword']?>"><img src="images/list.gif" border="0" alt="Listing Record">&nbsp;List   </a></td>
				   ...
				   <TBODY> 
				     ...
				     <? 
						...
						while(list($key,$val) = each($languageArray)){
							if(count($result) != 0){
								$_POST['service_content_'.$key] = stripslashes(stripslashes(stripslashes($result[$n]['text_content'])));
								$_POST['service_status']  =  $result[$n]['text_status'];
							}
					?> 
