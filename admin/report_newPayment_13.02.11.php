	...
	*/
	...
	if($_POST['daterange'] == 1){
		...
		switch($_POST['dropdown1']){
			case 'today':
				if($tab == 2 || $tab == 3)//Cancel/Refund
					$whereSql .= " AND STR_TO_DATE( PC.date, '%Y-%m-%d' )  = '".$today."'";				
				elseif($tab == 4)
					$whereSql .= " AND STR_TO_DATE( PP.join_date, '%Y-%m-%d' )  = '".$today."'";
				elseif($tab == 5)
			...
			case 'yest':
				...
				if($tab == 2 || $tab == 3)//Cancel/Refund
					$whereSql .= " AND STR_TO_DATE( PC.date, '%Y-%m-%d' ) = '".$yesterday."'";
				elseif($tab == 4)
					$whereSql .= " AND STR_TO_DATE( PP.join_date, '%Y-%m-%d' ) = '".$yesterday."'";	
				elseif($tab == 5)
			...
			case 'last7':
				...
				if($tab == 2 || $tab == 3)//Cancel/Refund
					$whereSql .= " AND STR_TO_DATE( PC.date, '%Y-%m-%d' ) BETWEEN '".$requiredDate."' AND '".$today."'";
				elseif($tab == 4)
					$whereSql .= " AND STR_TO_DATE( PP.join_date, '%Y-%m-%d' ) BETWEEN '".$requiredDate."' AND '".$today."'";	
				elseif($tab == 5)
			...
			case 'thismonth':	
				...
				if($tab == 2 || $tab == 3)//Cancel/Refund
					$whereSql .= " AND MONTH(STR_TO_DATE( PC.date, '%Y-%m-%d' )) = '".$thisMonth."'  AND YEAR(STR_TO_DATE( PC.date, '%Y-%m-%d' )) = '".$thisYear."'";	
				elseif($tab == 4)
					$whereSql .= " AND MONTH(STR_TO_DATE( PP.join_date, '%Y-%m-%d' )) = '".$thisMonth."'  AND YEAR(STR_TO_DATE( PP.join_date, '%Y-%m-%d' )) = '".$thisYear."'";	
				elseif($tab == 5)
			...
			case 'lastmonth':	
				...
				if($tab == 2 || $tab == 3)//Cancel/Refund
					$whereSql .= " AND MONTH(STR_TO_DATE( PC.date, '%Y-%m-%d' )) = '".$lasMonth."'  AND YEAR(STR_TO_DATE( PC.date, '%Y-%m-%d' )) = '".$lasYear."'";
				elseif($tab == 4)
					$whereSql .= " AND MONTH(STR_TO_DATE( PP.join_date, '%Y-%m-%d' )) = '".$lasMonth."'  AND YEAR(STR_TO_DATE( PP.join_date, '%Y-%m-%d' )) = '".$lasYear."'";
				elseif($tab == 5)
				...
				if($tab == 2 || $tab == 3)//Cancel/Refund
					$whereSql .= " AND YEAR(STR_TO_DATE( PC.date, '%Y-%m-%d' )) = '".$lasYear."'";	
				elseif($tab == 4)
					$whereSql .= " AND YEAR(STR_TO_DATE( PP.join_date, '%Y-%m-%d' )) = '".$lasYear."'";	
				elseif($tab == 5)
		...
		if(count($errorMsg) == 0){
			if($startDate != $endDate){
				if($tab == 2 || $tab == 3)
					$whereSql 	 .= " AND STR_TO_DATE( PC.date, '%Y-%m-%d' ) BETWEEN '".$startDate."' AND '".$endDate."'";
				elseif($tab == 4)
					$whereSql 	 .= " AND STR_TO_DATE( PP.join_date, '%Y-%m-%d' ) BETWEEN '".$startDate."' AND '".$endDate."'";
				elseif($tab == 5)
				...
				if($tab == 2 || $tab == 3)
					$whereSql .= " AND STR_TO_DATE( PC.date, '%Y-%m-%d' ) = '".$startDate."'";
				elseif($tab == 4)
					$whereSql .= " AND STR_TO_DATE( PP.join_date, '%Y-%m-%d' ) = '".$startDate."'";
				elseif($tab == 5)
	...
	if($tab == 2)
	{
		$query		=	"SELECT count(*) AS total_condition	FROM payment_transactions AS PC
INNER JOIN payment AS PS ON PC.payment_id = PS.payment_id
INNER JOIN user_master AS UM ON UM.user_id = PC.user_id WHERE PS.payment_status='0' AND PS.version='New' AND PC.status='CANCELLED'  $whereSql";
	}
	elseif($tab == 3)
	{
		$query		=	"SELECT count(*) AS total_condition	FROM payment_transactions AS PC
INNER JOIN payment AS PS ON PC.payment_id = PS.payment_id
INNER JOIN user_master AS UM ON UM.user_id = PC.user_id WHERE PS.payment_status='1' AND PS.version='New' AND PC.status='REFUND'  $whereSql";
	}
	elseif($tab == 4)
	{
		$query		=	"SELECT count(*) AS total_condition	FROM payment_paybox AS PP
INNER JOIN user_master AS UM ON UM.user_id = PP.user_id WHERE UM.user_status !=0 AND PP.status='ACTIVE'  $whereSql";
	}
	elseif($tab == 5)
	{
		$query		=	"SELECT count(*) AS total_condition	FROM payment_paybox AS PP
INNER JOIN user_master AS UM ON UM.user_id = PP.user_id WHERE UM.user_status !=0 AND PP.status	!=	'ACTIVE'  $whereSql";
	...
	{
		/*$query		=	"SELECT count(*) AS total_condition	FROM payment_paybox AS PP
INNER JOIN payment AS PS ON PP.user_id = PS.payment_userid
INNER JOIN user_master AS UM ON UM.user_id = PP.user_id WHERE PS.payment_status='1' AND PS.version='New' AND PP.status='ACTIVE'  $whereSql";*/
		$query		=	"SELECT count(*) AS total_condition	FROM payment AS PS		 

LEFT JOIN user_master AS UM ON UM.user_id = PS.payment_userid WHERE PS.payment_status='1' AND PS.version='New' $whereSql";
	}
	...
	if($total_condition['total_condition']>0){
		...
		$sql = "SELECT UM.user_fname, UM.user_lname,UM.user_email,PC.* 					
					FROM payment_transactions AS PC										
					INNER JOIN payment AS PS ON PC.payment_id = PS.payment_id
					INNER JOIN user_master AS UM ON UM.user_id = PC.user_id WHERE PS.payment_status='0' AND PS.version='New' AND PC.status='CANCELLED'
					$whereSql
					";
		}
		elseif($tab == 3)
		...
		$sql = "SELECT UM.user_fname, UM.user_lname,UM.user_email,PC.* 					
					FROM payment_transactions AS PC										
					INNER JOIN payment AS PS ON PC.payment_id = PS.payment_id
					INNER JOIN user_master AS UM ON UM.user_id = PC.user_id WHERE PS.payment_status='1' AND PS.version='New' AND PC.status='REFUND'
					$whereSql
					";
		}
		elseif($tab == 4)
		{
		$sql = "SELECT UM.user_fname, UM.user_lname,UM.user_email,PP.join_date,PP.status,PP.brand 					
					FROM payment_paybox AS PP															
					INNER JOIN user_master AS UM ON UM.user_id = PP.user_id WHERE UM.user_status !=0 AND PP.status='ACTIVE' $whereSql";
		}
		elseif($tab == 5)
		{
		$sql = "SELECT UM.user_fname, UM.user_lname,UM.user_email,PP.unsubscribed_date,PP.status,PP.brand 					
					FROM payment_paybox AS PP															
					INNER JOIN user_master AS UM ON UM.user_id = PP.user_id WHERE UM.user_status !=0 AND PP.status	!=	'ACTIVE' $whereSql";
		}
		else
		{
			/*$sql = "SELECT UM.user_fname, UM.user_lname,UM.user_email,PS.payment_amount,PS.payment_currency,PS.payment_expdate,UM.user_id, PC.date AS payment_date,PC.trans_refrns_id 					
					FROM payment_paybox AS PP										
					INNER JOIN payment AS PS ON PP.user_id = PS.payment_userid
					INNER JOIN payment_transactions AS PC ON PS.payment_id = PC.payment_id
					INNER JOIN user_master AS UM ON UM.user_id = PP.user_id WHERE PS.payment_status='1' AND PS.version='New' AND PC.status = 'PAID' AND PP.status='ACTIVE'
					$whereSql
					";	*/
			$sql = "SELECT UM.user_fname, UM.user_lname,UM.user_email,PS.payment_amount,PS.payment_currency,PS.payment_expdate,UM.user_id, PC.date AS payment_date,PC.trans_refrns_id 					
					FROM payment AS PS																				
					LEFT JOIN payment_transactions AS PC ON PS.payment_id = PC.payment_id
					LEFT JOIN user_master AS UM ON UM.user_id = PS.payment_userid WHERE PS.payment_status='1' AND PS.version='New' AND PC.status = 'PAID' $whereSql
					";	
		}
	...
	//$sql = $sqlStart.$fromSql.$whereSql." GROUP BY user_master.user_id ORDER BY user_master.user_doj DESC";
		...
		else
		{
			$paging_query	= "SELECT count(PS.payment_userid) 
					FROM payment AS PS															
					LEFT JOIN user_master AS UM ON UM.user_id = PS.payment_userid WHERE PS.payment_status='1' AND PS.version='New'  
					$whereSql";
		}
              ...
              <tr> 
				...
				<TABLE cellSpacing=0 cellPadding=0 border=0 align="center">
				  ...
				  <tr>
				    ...
				    <td width="117" height="27" align="center" valign="bottom" class="sectionHeading"><a href="excel_newpayment.php?inf=<? echo base64_encode($sql);?>&tab=<?php echo $tab;?>&whereSql=<?php echo base64_encode($whereSql);?>"><img src="../images/sports/english/download.gif" style="float:right" border="0"></a></td>
				  </tr>
...
<select name="toY">
  <?
	...
	<ul>		
        <li><a href="report_newPayment.php?pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=1"><span>Payments</span></a></li>
		<li><a href="report_newPayment.php?pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=2"><span>Cancel</span></a></li>
		<li><a href="report_newPayment.php?pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=3"><span>Refund</span></a></li>
		<li><a href="report_newPayment.php?pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=4"><span>Members</span></a></li>
        <li><a href="report_newPayment.php?pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=5"><span>Unsubscribed </span></a></li>
                  ...
                  <?php if($tab == 2){
					  	if(count($result)>0)
				  		{						 
						  $amountQuery	=	"SELECT sum( PS.payment_amount ) AS Total, PS.payment_currency FROM  		 											payment_transactions AS PC
 											INNER JOIN payment AS PS ON PC.payment_id = PS.payment_id
 											INNER JOIN user_master AS UM ON UM.user_id = PC.user_id WHERE                    						PS.payment_status='0' AND PS.version='New' AND PC.status='CANCELLED'   											$whereSql GROUP BY PS.payment_currency";					
						  $resultAmount	=	$objDb->_getList($amountQuery);	
						  $PlanQuery	=	"SELECT sum( PS.payment_amount ) AS Total,COUNT( PS.payment_amount )  																				 											As count,PS.payment_currency,PS.payment_amount FROM                             				payment_transactions AS PC INNER JOIN payment AS PS ON PC.payment_id = 											PS.payment_id INNER JOIN user_master AS UM ON UM.user_id = PC.user_id  											WHERE PS.payment_status='0' AND PS.version='New' AND                                           	PC.status='CANCELLED'  $whereSql GROUP BY PS.payment_amount ORDER BY   											`PS`.`payment_currency` DESC";					
			...
			?><tr  <? if(($key%2) ==1){?> bgcolor="#FFFFFF" <? } ?> >
                              ...
                              <TD width="14%" align="center" style="padding-left:10px;"><?php echo $currency;?></TD>
                              
                              <TD width="18%" align="center" style="padding-left:10px;"><?php echo $val['date'];?></TD>
                            </tr>
			...
			?>							
					...
					<tr><?php if($noOfPage > 1) { ?>
						...
						<a href="report_newPayment.php?pageNo=1&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=<?=$tab?>">
						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>
						<a href="report_newPayment.php?pageNo=<?=$prev?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=<?=$tab?>">
						...
						<select name="pageNo" class="paragraph"  onChange="form.submit()">
							...
							?>
							 ...
							 of <?=$noOfPage?>]
							 <a href="report_newPayment.php?pageNo=<?=$next?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=<?=$tab?>">
							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>
							<a href="report_newPayment.php?pageNo=<?=$noOfPage?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&param=<?=$param?>&tab=<?=$tab?>">
			...
			?><tr  <? if(($key%2) ==1){?> bgcolor="#FFFFFF" <? } ?> >
                              ...
                              <TD width="11%" align="center" style="padding-left:10px;"><?php echo $currency;?></TD>
                              
                              <TD width="15%" align="center" style="padding-left:10px;"><?php echo $val['date'];?></TD>
                            </tr>
			...
			?><tr  <? if(($key%2) ==1){?> bgcolor="#FFFFFF" <? } ?> >
                              ...
                              <TD width="11%" align="center" style="padding-left:10px;"><?php if($val['status']	== 'ACTIVE') echo "<img src=\"images/y.gif\" width=\"14\" height=\"14\">"; else echo "<img src=\"images/n.gif\" width=\"14\" height=\"14\">";?></TD>
			...
			?><tr  <? if(($key%2) ==1){?> bgcolor="#FFFFFF" <? } ?> >
                              ...
                              <TD width="29%" height="19" align="left" style="padding-left:10px;"><?php echo $val['user_email'];?></TD>
                              <TD width="16%" align="center" style="padding-left:10px;"><?php echo $val['brand'];?></TD>
                              <TD width="11%" align="center" style="padding-left:10px;"><?php echo $val['status'];?></TD>
			...
			?>							
				  ...
				  if(count($result)>0)
				  {
					  $amountQuery	=	"SELECT sum( PS.payment_amount ) AS Total, PS.payment_currency
										FROM payment AS PS LEFT JOIN user_master AS UM ON UM.user_id = 																																																																																	 										PS.payment_userid WHERE PS.payment_status = '1' AND PS.version = 'New'
										$whereSql GROUP BY PS.payment_currency";
					  $resultAmount	=	$objDb->_getList($amountQuery);	
					  $PlanQuery	=	"SELECT sum( PS.payment_amount ) AS Total,COUNT( PS.payment_amount ) As 		 										count,PS.payment_currency,PS.payment_amount
										FROM payment AS PS LEFT JOIN user_master AS UM ON UM.user_id = 																																																																																	 										PS.payment_userid WHERE PS.payment_status = '1' AND PS.version = 'New'
										$whereSql GROUP BY PS.payment_amount ORDER BY `PS`.`payment_currency`  																									 										DESC";
					  $resultPlan	=	$objDb->_getList($PlanQuery);	
					  ?> 
                  	<fieldset>
