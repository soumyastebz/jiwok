...
if($_POST['report'] == 'country' && $_POST['user_country']!="" && $_POST['user_country']!="0"){
	...
	$whereSql .= " AND c.user_country = ". $_POST['user_country']." ";
}
...
$myquery	="SELECT b.payment_amount,b.payment_currency,a.referred_medium, c.user_fname,c.user_lname,c.user_alt_email AS user_email FROM 

payment  AS b   LEFT JOIN jiwok_referrals AS  a ON (a.user_id = b.payment_userid AND 

a.referrer_user_id	='0' ) LEFT JOIN user_master AS c ON c.user_id=a.user_id ". $whereSql .$fltrCond. " and b.payment_status=1 AND	b.version = 'New' ";
...
*/
$myquery_totalRecs = mysql_num_rows(mysql_query($myquery));
...
if($myquery_totalRecs <= 0)
	$errMsg1 = "No Records";
$sqlStartAll_new =	"select IFNULL(sum(b.payment_amount),0) as amtPaid from payment b
where b.payment_userid in (select a.user_id from jiwok_referrals a  LEFT JOIN user_master AS c ON c.user_id=a.user_id ".$whereSql.$fltrCond." ) and b.payment_status=1 AND	b.version = 'New' ";
$resultAll_new	=$objDb->_getList($sqlStartAll_new);//for total display
              ...
              <tr> 
			   ...
			   <form name="reportFrm" action="#" method="post">
                  ...
                  <tr><td>
                      ...
                      <option value="oxygem" <?php if($flrtVal=="oxygem"){?>selected<?php } ?>>oxygem</option>
                      <option value="runningheroes" <?php if($flrtVal=="runningheroes"){?>selected<?php } ?>>runningheroes</option>
                      </select>
...
<TR class="tableHeaderColor">
                        ...
                        <TD width="10%" align="center" >Amount Paid</TD>
                            ...
                            <tr  <? if(($key%2) ==0){?> bgcolor="#FFFFFF" <? } ?> >
                              ...
                              <TD width="10%" height="19" align="left" style="padding-left:10px;"><? echo $val['payment_amount']." ".$val['payment_currency']?></TD>
                ...
                <table cellspacing=0 cellpadding=0 width=553 border=0 bgcolor="#FFFFFF" >

                                <tbody>		   <?php
							if(count($resultAll_new)>0){
							?>
							<tr>

                              <TD width="6%" height="19" align="center"></TD>

                              <TD width="21%" height="19" align="left" 

style="padding-left:10px;"><b>TOTAL</b></TD>
                              <TD width="26%" height="19" align="left" style="padding-left:10px;"></TD>
                              <TD width="17%" height="19" align="left" style="padding-left:10px;"></TD>
                              <TD width="17%" height="19" align="left" style="padding-left:10px;"></TD>
                             
                              <TD width="10%" height="19" align="left" style="padding-left:10px;"><? echo 

round($resultAll_new[0]['amtPaid'],2);?></TD>
