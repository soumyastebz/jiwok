...
/**************************************************************************** 
   ...
   Module 		::> Admin-slider Management
   ...
   This  code userd to list the all slides in home page .
...
*****************************************************************************/
	...
	*/
	...
	if($_REQUEST['field']){
		...
		$field = "slide_manager_home.".$_REQUEST['field'];

		$type = $_REQUEST['type'];

	}else{

		$field = "slide_content_name";
...
//~ 
	...
	$totalRecs = $objService->_getTotalCountNew($lanId);
	...
	if($_REQUEST['pageNo']){
		...
		$result =  $objService->_showPageNew($totalRecs,$_REQUEST['pageNo'],$_REQUEST['maxrows'],$field,$type);
...
<tr> 
              ...
              <tr> 
			   ...
			   <form name="frmservices" action="list_slides_home.php" method="post">
				  ...
				  <TABLE class="listTableStyle" cellSpacing=1 cellPadding=2 width="553">
				   ...
				   <TBODY> 
					   ...
					   <TR class="tableHeaderColor">
						...
						<a href="list_slides_home.php?field=slide_content_name&type=asc&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>"><img src="images/up.gif" border="0" alt="Ascending Sort">&nbsp;</a>

						<a href="list_slides_home.php?field=slide_content_name&type=desc&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>"><img src="images/down.gif" border="0" alt="Descending Sort"></a>
					...
					<!--	<TD width="8%" align="center" >Status</TD> -->
					 ...
					 <?php }
						...
						?>
							...
							<TD><?=stripslashes(stripslashes(stripslashes($row['slide_content_name'])));?></TD>

							<TD align="center">

								<a href = "addedit_slide_home.php?slideId=<?=$row['manager_id']?>&pageNo=<?=$_REQUEST['pageNo']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&type=<?=$_REQUEST['type']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>&action=edit" class="smallLink">Edit</a>&nbsp;
				...
				<!--<table cellspacing=0 cellpadding=0 width=553 border=0 class="topColor">
					...
					<tr>
						...
						<a href="list_slides_home.php?pageNo=1&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>">

						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>

						<a href="list_slides_home.php?pageNo=<?=$prev?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>">
						...
						<select name="pageNo" class="paragraph"  onChange="form.submit()">
							...
							?>
							 ...
							 <a href="list_slides_home.php?pageNo=<?=$next?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&field=<?=$_REQUEST['field']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>">

							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>

							<a href="list_slides_home.php?pageNo=<?=$noOfPage?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output($_REQUEST['keyword'])?>">
