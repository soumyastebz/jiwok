...
function getReportOfSubscribers($type='',$condition='',$sort,$lower_limit,$maxrows)
{
...
FROM user_master";
		echo $sql;*/
	$curdate = date('Y-m-d');
    $query = "SELECT user_master.user_id as user_id,user_master.user_fname as user_fname,user_master.user_lname,user_master.user_email,user_master.user_gender,FLOOR((TO_DAYS(NOW())- TO_DAYS(STR_TO_DATE( user_master.user_dob, '%d/%m/%Y' ))) / 365.25) as age,user_country,brand_master.brand_name,user_master.user_language
			FROM user_master
			LEFT JOIN brand_user ON ( user_master.user_id = brand_user.user_id )
			LEFT JOIN brand_master ON ( brand_master.brand_master_id = brand_user.brand_master_id )
			LEFT JOIN payment ON user_master.user_id = payment.payment_userid
...
function getOneEuroOriginDiscountCode($userid)
{
	...
	$query = "SELECT discount_code as discountcode
			...
			WHERE discount_users.user_id ='".$userid."'
			 AND discount_users.user_id !=0
			 AND discount_users.user_id IS NOT NULL
			AND discount_users.discount_type='DISC'
			AND discount_users.payment_status='success' 
			GROUP BY discount_users.user_id ORDER BY start_date ASC";
...
function checkGiftCodeTransaction($userid)
{
	...
	$query = "SELECT MIN(usedate) as startdate
			 ...
			 INNER JOIN gift_member ON gift_userdetails.purchaseid = gift_member.purchaseid
			 INNER JOIN payment ON gift_userdetails.payment_id = payment.payment_id
			 WHERE gift_userdetails.user_id='".$userid."' AND gift_userdetails.user_id!=0 AND gift_userdetails.user_id IS NOT NULL AND gift_userdetails.payment_id IS NOT NULL AND gift_userdetails.usedate!='0000-00-00' AND gift_userdetails.usedate IS NOT NULL AND gift_userdetails.payment_id!=0 AND payment.payment_status=1 AND payment_amount >=7.9 and payment_amount!=0 AND payment_amount IS NOT NULL GROUP BY gift_userdetails.user_id ORDER BY usedate ASC";
...
function checkNormalPaymentTransaction($userid)
{
	...
	$query = "SELECT payment.payment_id as paymentid,payment.payment_date as paymentdate
			 ...
			 AND payment.payment_date!='0000-00-00'
			 AND payment.payment_status=1
			 AND (payment_amount='7.9' or payment_amount='9.9' ) and payment_amount!=0 AND payment_amount IS NOT NULL
		     GROUP BY payment.payment_userid ORDER BY payment.payment_date ASC";
