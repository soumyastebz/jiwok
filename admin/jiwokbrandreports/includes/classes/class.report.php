...
class Report{
	...
	public function getAllBrandName(){
		...
		public function getOxyBrandName($brand_name = null){
			$sql 	= "SELECT id,camp_name FROM campaign_manage where status=1 AND LOWER(camp_name) LIKE '%$brand_name%'";
		...
		public function getAllOxyCodes($brand_name = null){
			$sql 	= "SELECT distinct 	gift_card_no FROM campaign_reports where status=1 AND LOWER(campaign_name) LIKE '%$brand_name%'";
