...
//	error_reporting(0);
	...
	*/
	...
	if(isset($_POST['report']) && ( $_POST['report']=='paid' || $_POST['report']=='free')){
		if(createPaidUsers()===true){
			if($_POST['report']=='paid'){
				$payment_qry	=	" INNER JOIN paid_users_temp AS PA ON PA.payment_userid = PQ.user_id ";
			}elseif($_POST['report']=='free'){
				$whereSql	.=	" AND PA.payment_userid IS NULL ";
				$payment_qry	=	" LEFT JOIN paid_users_temp AS PA ON PA.payment_userid = PQ.user_id ";
			}
		}
	}
	//***********for polishadmin
	...
	$query	= "SELECT count(*) AS total	
				...
				INNER JOIN user_master AS UM ON UM.user_id = PQ.user_id ".$joinquery;
	...
	if($total_condition['total_condition']>0){
		...
		$sql = "SELECT UM.user_fname, UM.user_lname, PQ.user_id, 
					...
					INNER JOIN user_master AS UM ON UM.user_id = PQ.user_id
					$whereSql $joinquery
					GROUP BY PQ.user_id ORDER BY COUNT(PQ.user_id) DESC, UM.user_fname ASC";
	//die($sql);
	//echo $sql;
	...
	//$sql = $sqlStart.$fromSql.$whereSql." GROUP BY user_master.user_id ORDER BY user_master.user_doj DESC";
		...
		$paging_query	= "SELECT count(DISTINCT(PQ.user_id)) 
					FROM program_queue AS PQ 
					$payment_qry
					INNER JOIN user_master AS UM ON UM.user_id = PQ.user_id
					$whereSql $joinquery";
...
//////////////////////////////// PAGINATION //////////////////////////////////
	...
	$sql1	=	"SELECT PQ.user_id, PQ.workout_flex_id, WD.workout_title, 
							DATE_FORMAT(PQ.invoke_time, '%D %M %Y') AS invoke_time
						FROM program_queue AS PQ 
...
<select name="toY">
  <?
                  ...
                  <TABLE cellSpacing=1 cellPadding=2 width="553">
                    <TBODY>
					<TR >
                        <TD width="50%" align="left" class="sectionHeading">Total Generations :</TD>
				...
				?><tr>
					...
					<table width="100%" cellspacing="0" cellpadding="1">
					  <tr <? if(($k%2) ==1){?> bgcolor="#FFFFFF" <? } ?>>
						...
						<td width="30%"><?=$programs[$result[$i]['user_id']][$k]['invoke_time']?></td>
					  </tr>
					</table>
					</td>
				</tr>
