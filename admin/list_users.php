...
*****************************************************************************/
	...
	*/
	...
	$totalRecs = $objUsers->_getTotalCount($searchQuery,$lanId);
	if($totalRecs <= 0)
		$errMsg = "No Records";
	...
	if($_REQUEST['pageNo']){
		...
		$result =  $objUsers->_showPage($totalRecs,$_REQUEST['pageNo'],$_REQUEST['maxrows'],$field,$type,$searchQuery);
	}
	else{
	...
	if($totalRecs <= $_REQUEST['pageNo']*$_REQUEST['maxrows'])
	{
		...
		$endNo = $totalRecs;
		$displayString = "Viewing $startNo to $endNo of $endNo users";
	...
	else
	{
		...
		$displayString = "Viewing $startNo to $endNo of $totalRecs users";
...
<tr> 
              ...
              <tr> 
			   ...
			   <form name="frmfaqs" action="list_users.php" method="post">
				  ...
				  <TABLE class="listTableStyle" cellSpacing=1 cellPadding=2 width="553">
				   <TBODY> 
					   <TR class="tableHeaderColor">
						...
						<TD width="48%" >Program&nbsp;Flexid
						
						<a href="list_users.php?field=program_flex_id&type=asc&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$objGen->_output(stripslashes($_REQUEST['keyword']))?>"><img src="images/up.gif" border="0" alt="Ascending Sort">&nbsp;</a>
						<a href="list_users.php?field=program_flex_id&type=desc&maxrows=<?=$_REQUEST['maxrows']?>&pageNo=<?=$_REQUEST['pageNo']?>&keyword=<?=$objGen->_output(stripslashes($_REQUEST['keyword']))?>"><img src="images/down.gif" border="0" alt="Descending Sort"></a>						</TD>
					 ...
					 <?php }
						...
						?>
							...
							<TD align="center">
								...
								<a href = "edit_users.php?pgid=<?=$row['program_flex_id']?>&pageNo=<?=$_REQUEST['pageNo']?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&type=<?=$_REQUEST['type']?>&keyword=<?=$objGen->_output(stripslashes($_REQUEST['keyword']))?>&action=view" class="smallLink"">Edit</a>&nbsp;
				...
				<table cellspacing=0 cellpadding=0 width=553 border=0 class="topColor">
					...
					<tr>
						...
						<a href="list_users.php?pageNo=1&type=<?=$_REQUEST['type']?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output(stripslashes($_REQUEST['keyword']))?>">
						<img src="images/mail-arrowfg.gif" border=0 width="20" height="20" align="absmiddle" alt="First Page"></a>
						<a href="list_users.php?pageNo=<?=$prev?>&type=<?=$_REQUEST['type']?>&langId=<?=$lanId?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output(stripslashes($_REQUEST['keyword']))?>">
						...
						<select name="pageNo" class="paragraph"  onChange="form.submit()">
							...
							?>
							 ...
							 of <?=$noOfPage?>]
							 <a href="list_users.php?pageNo=<?=$next?>&type=<?=$_REQUEST['type']?>&field=<?=$_REQUEST['field']?>&maxrows=<?=$_REQUEST['maxrows']?>&keyword=<?=$objGen->_output(stripslashes($_REQUEST['keyword']))?>">
							<img src="images/mail-arrowrg.gif" border=0 width="20" height="20" align="absmiddle" alt="Next Page"></a>
							<a href="list_users.php?pageNo=<?=$noOfPage?>&type=<?=$_REQUEST['type']?>&maxrows=<?=$_REQUEST['maxrows']?>&field=<?=$_REQUEST['field']?>&keyword=<?=$objGen->_output(stripslashes($_REQUEST['keyword']))?>">
